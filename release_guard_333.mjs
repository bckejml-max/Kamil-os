...
if(releaseMajor>=345){for(const symbol of ['DEFER345','__KAMIL_DEFERRED345__','scheduleDeferred345','kamil:deferred345-complete'])if(!boot.includes(symbol))fail(`OS345 deferred boot missing ${symbol}`);for(const path of ['./performance330.js','./ticketQa332.js'])if(!boot.includes(path))fail(`OS345 deferred boot missing ${path}`);if(!fs.existsSync(new URL('./e2e_os345_deferred_boot.spec.mjs',import.meta.url)))fail('missing OS345 browser regression')}
if(releaseMajor>=346){const loaderPath='./js/ticketOnDemand346.js';if(!fs.existsSync(new URL(loaderPath,import.meta.url)))fail(`missing OS346 file ${loaderPath}`);else{const loader=read(loaderPath);for(const symbol of ['STATE346','__KAMIL_TICKET_ON_DEMAND346__','loadTicketDesk346','ticket-on-demand346'])if(!loader.includes(symbol))fail(`OS346 ticket loader missing ${symbol}`)}if(!boot.includes("./ticketOnDemand346.js"))fail('OS346 boot missing ticketOnDemand346');if(boot.includes("optionalImport('./ticketDesk331.js'"))fail('OS346 must not eager-load ticketDesk331');if(!fs.existsSync(new URL('./e2e_os346_ticket_on_demand.spec.mjs',import.meta.url)))fail('missing OS346 browser regression')}
