...
if(releaseMajor>=345){const loaderPath='./js/ticketBoot345.js';if(!fs.existsSync(new URL(loaderPath,import.meta.url)))fail(`missing OS345 file ${loaderPath}`);else{const loader=read(loaderPath);for(const symbol of ['installTicketBoot345','ticketDesk331.js','ticketQa332.js','__KAMIL_TICKET_BOOT345__'])if(!loader.includes(symbol))fail(`OS345 ticket boot missing ${symbol}`)}if(!boot.includes("./ticketBoot345.js"))fail('OS345 boot missing ./ticketBoot345.js');if(boot.includes("optionalImport('./ticketDesk331.js'")||boot.includes("optionalImport('./ticketQa332.js'"))fail('OS345 ticket desk/QA must stay out of critical boot');if(!fs.existsSync(new URL('./e2e_os345_lazy_tickets.spec.mjs',import.meta.url)))fail('missing OS345 browser regression')}
