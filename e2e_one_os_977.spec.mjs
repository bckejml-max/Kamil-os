...
test('OS977 makes One OS the primary More surface',async({page})=>{
 ...
 await page.getByRole('button',{name:'Pokročilé / legacy'}).click();
 await expect(page.getByText('Pokročilé / legacy · OS969')).toBeVisible();
 await expect(page.getByText('Operator Center · OS717')).toBeVisible();
});
