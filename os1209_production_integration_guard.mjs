...
const [market,ledger,integrity,gmail,recovery,ticket,migration,betApi,authBridge]=await Promise.all([
...
assert.ok(betApi.includes('getBettingLedger543(req)'),'betting API must authenticate reads');
assert.ok(betApi.includes('mutateBettingLedger543(await readBody(req),req)'),'betting API must authenticate mutations');
...
console.log('OS1209 PASS: critical production wiring for provider, RLS betting, Gmail, ticket diagnostics and recovery is present');
