...
const [market,ledger,integrity,gmail,recovery,ticket,migration,authBridge,vercel]=await Promise.all([
 ...
 read('./supabase/migrations/0034_betting_ledger543.sql'),
 read('./js/apiAuthBridge1210.js'),
 read('./vercel.json')
]);
const marketHistory=await read('./api/market-history.js');
...
for(const token of ["REQUIRED_REVISION='0034_betting_ledger543.sql'",'AUTH_REQUIRED','REMOTE_BETTING_LEDGER_MIGRATION_REQUIRED','BET_ALREADY_SETTLED','BET_SETTLEMENT_RACE','supabase_rls'])assert.ok(ledger.includes(token),`betting ledger RLS contract missing ${token}`);
for(const token of ['enable row level security','auth.uid()','BETTING_LEDGER_SETTLED_IMMUTABLE','primary key (user_id, id)'])assert.ok(migration.includes(token),`betting migration missing ${token}`);
assert.ok(marketHistory.includes('getBettingLedger543(req)'),'shared betting route must authenticate reads');
assert.ok(marketHistory.includes('mutateBettingLedger543(await readBody(req),req)'),'shared betting route must authenticate mutations');
assert.ok(vercel.includes('"destination": "/api/market-history?source=ledger543"'),'betting API must share existing serverless function on Hobby');
assert.ok(authBridge.includes("headers.set('Authorization',`Bearer ${token}`)"),'browser API auth bridge missing bearer forwarding');
...
console.log('OS1209 PASS: critical production wiring for provider, shared RLS betting, Gmail, ticket diagnostics and recovery is present');
