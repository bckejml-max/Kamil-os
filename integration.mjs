...
assert(store.get().meta.schemaVersion===40,'schema migrated to v40');assert(store.get().projects[0].name==='Nová Zbrojovka','legacy work data preserved');assert(store.get().emergencyFile.contacts.length===1&&store.get().emergencyFile.assets.length===1,'emergency data preserved');assert(store.get().personalInbox.items.length===1&&store.get().assetBook.items.length===1&&store.get().personalGoals.items.length===1,'29 data preserved');
...
assert(personalQuery('co končí do 30 dní',store.get(),{},ref),'copilot deadline query');assert(personalQuery('kolik můžu bezpečně investovat',store.get(),{},ref),'copilot investment query');assert(personalQuery('ukaž cíle a fondy',store.get(),{},ref),'goals query');
