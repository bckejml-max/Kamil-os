...
const USER_AGENT='Mozilla/5.0 (compatible; KamilOS/32.4; +https://kamil-os-smoke.vercel.app/)';
const sleep=ms=>new Promise(r=>setTimeout(r,ms));
function requested(req){const raw=req.query?.symbols??req.query?.symbol??'',values=Array.isArray(raw)?raw:String(raw).split(','),out=[];for(const v of values){const s=quoteSymbol32(v);if(s&&!out.includes(s))out.push(s);if(out.length>=MARKET_QUOTE_SOURCE_32.maxSymbols)break}return out}
...
export default async function handler(req,res){
 ...
 return res.status(quotes.length?200:502).json({ok:quotes.length>0,provider:MARKET_QUOTE_SOURCE_32.provider,fetchedAt:new Date().toISOString(),requested:symbols,quotes,errors,contract:{factsOnly:true,investmentAction:false,changesDecisionAction:false,thirdPartyPublic:true}});
}
