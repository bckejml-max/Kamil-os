...
test('Canonical Personal Home stays personal even when legacy market state exists',async({page})=>{
 ...
 await expect(page.locator('.ux65-primary')).toHaveCount(1);
 const diag=await page.evaluate(()=>({text:document.querySelector('#todayView')?.textContent||'',marketHome:window.__KAMIL_MARKET_HOME_560_LAST__||null,queue:window.__KAMIL_ACTION_QUEUE_559_LAST__||null}));
