...
test('Canonical Personal Home stays personal even when legacy market state exists',async({page})=>{
 ...
 const diag=await page.evaluate(()=>({today:window.__KAMIL_PERSONAL_UX_650_LAST__||null,text:document.querySelector('#todayView')?.textContent||'',marketHome:window.__KAMIL_MARKET_HOME_560_LAST__||null,queue:window.__KAMIL_ACTION_QUEUE_559_LAST__||null}));
