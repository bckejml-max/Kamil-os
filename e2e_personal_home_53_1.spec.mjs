...
test('Personal Market Home 53.3 is fast and focuses only XTB plus tickets',async({page})=>{
 ...
 const diag=await page.evaluate(()=>({home:window.__KAMIL_PERSONAL_HOME_531_LAST__||null,market:window.__KAMIL_MARKET_TOP3_533_LAST__||null,suite:window.__KAMIL_SUITE_530_LAST__||null,assistant:window.__KAMIL_ASSISTANT_530_LAST__||null,text:document.querySelector('#todayView')?.textContent||''}));
 expect(diag.home).not.toBeNull();expect(diag.home.ms).toBeLessThan(200);expect(diag.market).not.toBeNull();expect(diag.market.ms).toBeLessThan(200);
 ...
 expect(diag.text).toContain('SMART TOP 3');expect(diag.text).toContain('XTB');expect(diag.text).toContain('VSTUPENKY');
});
