...
const state={
 ...
 personalInbox:{items:[{id:'i1',title:'Inbox',status:'NEW'}]},assetBook:{items:[{id:'asset1',title:'Auto',status:'ACTIVE'}]},
...
assert(env.format===BACKUP_FORMAT&&env.schemaVersion===39,'envelope metadata');
assert(!JSON.stringify(env).includes('UNDO-SECRET-SHOULD-NOT-EXPORT'),'undo historical secret not exported');
assert(env.payload.personalAdmin.items[0].title==='Pojištění','user records preserved');assert(env.payload.personalInbox.items.length===1&&env.payload.assetBook.items.length===1,'Autopilot records preserved');
...
health=backupHealth(state,{lastBackupAt:'2026-08-18T10:00:00+02:00'},ref);assert(health.status==='OK','fresh backup detected');assert(health.counts.emergencyContacts===1&&health.counts.personalAdmin===1&&health.counts.personalInbox===1&&health.counts.assets===1,'personal coverage counted');
