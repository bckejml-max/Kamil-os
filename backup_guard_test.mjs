...
const state={
 ...
 personalSpending:{transactions:[{id:'tx1',date:'2026-08-19',description:'Lidl',amount:-500,currency:'CZK'}]},importCenter:{history:[{id:'imp1',at:'2026-08-20T08:00:00Z',source:'SPENDING',imported:1}]},
...
assert(env.format===BACKUP_FORMAT&&env.schemaVersion===41,'envelope metadata');
assert(!JSON.stringify(env).includes('UNDO-SECRET-SHOULD-NOT-EXPORT'),'undo historical secret not exported');
assert(env.payload.personalAdmin.items[0].title==='Pojištění','user records preserved');assert(env.payload.personalInbox.items.length===1&&env.payload.assetBook.items.length===1&&env.payload.personalGoals.items.length===1,'Autopilot records preserved');assert(env.payload.personalSpending.transactions.length===1&&env.payload.importCenter.history.length===1,'Smart Import records preserved');
...
health=backupHealth(state,{lastBackupAt:'2026-08-18T10:00:00+02:00'},ref);assert(health.status==='OK','fresh backup detected');assert(health.counts.emergencyContacts===1&&health.counts.personalAdmin===1&&health.counts.personalInbox===1&&health.counts.assets===1&&health.counts.goals===1&&health.counts.transactions===1&&health.counts.imports===1,'personal + import coverage counted');
