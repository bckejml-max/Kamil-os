...
test('Daily Personal Assistant Today stays compact on desktop and mobile',async({page})=>{
 ...
 expect(desktop.primary).toBeLessThanOrEqual(1);expect(desktop.secondary).toBeLessThanOrEqual(2);expect(desktop.context).toBe(4);expect(desktop.dataHealth).toBe(0);expect(desktop.width).toBeLessThanOrEqual(930);expect(desktop.text).not.toContain('Koncert A');expect(desktop.text).toContain('Dnes hotovo');
