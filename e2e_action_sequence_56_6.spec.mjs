...
test('Action Sequence 56.6 recomputes the next step over virtual post-action state without writes',async({page})=>{
 ...
 const result=await page.evaluate(async()=>{const m=await import('./js/actionSequence566.js'),s=JSON.parse(localStorage.getItem('kamil-os-state'));return m.actionSequence566(s,3)});
 expect(result.total).toBeGreaterThanOrEqual(2);expect(result.steps.length).toBeLessThanOrEqual(3);
 expect(new Set(result.steps.map(x=>x.key)).size).toBe(result.steps.length);
 for(let i=0;i<result.steps.length;i++){expect(result.steps[i].sequence).toBe(i+1);expect(result.steps[i].recalcNote.length).toBeGreaterThan(0)}
 if(result.steps.length>1)expect(result.steps[1].stateBefore).toEqual(result.steps[0].stateAfter);
 expect(result.flowText).toMatch(/CZK|Bez potvrzeného cash flow/);
 const after=await page.evaluate(mutationFingerprint);expect(after).toEqual(before);
 const diag=await page.evaluate(()=>window.__KAMIL_ACTION_SEQUENCE_566_LAST__);expect(diag.steps).toBe(result.steps.length);expect(diag.ms).toBeLessThan(1000);
});

test('Action Sequence 56.6 stays lazy and opens only after explicit click',async({page})=>{
 const state=seed();await page.addInitScript(s=>localStorage.setItem('kamil-os-state',JSON.stringify(s)),state);await page.goto(BASE,{waitUntil:'domcontentloaded'});
 expect(await page.evaluate(()=>window.__KAMIL_ACTION_SEQUENCE_566_LAST__||null)).toBeNull();
 ...
 await expect(page.locator('#modalHost')).toContainText('ACTION SEQUENCE 56.6');
 await expect(page.locator('#modalHost')).toContainText(/Stav se nikam nezapisuje/i);
 expect(await page.evaluate(()=>window.__KAMIL_ACTION_SEQUENCE_566_LAST__||null)).not.toBeNull();expect(await page.evaluate(()=>!!document.querySelector('#platform43'))).toBe(false);
});
