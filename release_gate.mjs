...
store.mutate('první změna',s=>{s.personalAdmin.items.push({id:'x',title:'Test',status:'ACTIVE'})});store.mutate('druhá změna',s=>{s.personalAdmin.items[0].title='Test 2'});assert(store.get().undo.length===2,'undo keeps configured steps');assert(store.get().undo.every(x=>Array.isArray(x.state?.undo)&&x.state.undo.length===0),'undo snapshots never contain nested undo snapshots');
