...
function render(){
 ...
 host.innerHTML=`<div class="card"><div class="card-head"><div><div class="eyebrow">CORE V2 / LIVE BRAIN 32.3 + MARKET EDGE 32.4</div><h2>Source Trust</h2><p class="muted">Live signál smí změnit XTB nebo ticket rozhodnutí jen když má čerstvý čas, validní webový zdroj a confidence.</p></div><span class="status ${tone(s.status)}">${label(s.status)}</span></div><div class="metric-strip"><div class="metric"><span>Ověřené live</span><b>${s.trusted}</b></div><div class="metric"><span>Blokované live</span><b>${s.blocked}</b></div><div class="metric"><span>Celkem kandidátů</span><b>${s.total}</b></div></div>${row('XTB',s.xtb.trusted,`${s.xtb.total} kandidátů · ${s.xtb.blocked} blokováno`)}${row('Vstupenky',s.tickets.trusted,`${s.tickets.total} kandidátů · ${s.tickets.blocked} blokováno`)}${row('Bez validního zdroje',s.xtb.unsourced+s.tickets.unsourced)}${row('Zastaralé',s.xtb.stale+s.tickets.stale)}${row('Bez confidence',s.xtb.noConfidence+s.tickets.noConfidence)}${row('Neplatný / budoucí čas',s.xtb.invalidAsOf+s.tickets.invalidAsOf)}<div class="decision-note">SOURCE BACKED ONLY: bez validního zdroje se live kandidát nesmí vydávat za živé doporučení ani přepsat pravidlové AUTO rozhodnutí. 32.4 navíc umí získat faktickou SEC filing evidence; samotný filing ale nikdy automaticky nevytvoří BUY/SELL verdikt.</div></div>`;
}
