...
const priorityRows=(rows=[])=>rows.length?rows.map((x,i)=>`<div class="row"><div><b>${i+2}. ${h(x.title)}</b><div class="muted">${h(x.meta)}</div></div><button class="btn" data-priority-index="${i}">Otevřít</button></div>`).join(''):'<div class="empty success-empty">Žádné další silné priority.</div>';

export function renderTodayLite43(){
 if(fullModule){fullModule.renderToday?.();return}
 const host=qs('#todayView');if(!host)return;const started=performance.now(),token=String(++seq),s=store.get(),m=marketSnapshot(s),g=globalPrioritySnapshot(s,m),secondary=g.top.slice(1,6);host.dataset.todayLite43=token;
 ...
 <div class="card"><div class="card-head"><div><div class="eyebrow">DALŠÍ PRIORITY · 60.1</div><h2>Co následuje potom?</h2></div><button class="btn" data-today-detail>Otevřít detail Dnes</button></div>${priorityRows(secondary)}</div>
...
// Legacy compatibility markers: Dnes řeš to důležité. Zbytek může počkat. · Personal 45.5 drží Safe Core · data-life-dashboard · import('./lifeDashboard455.js') · Životní dashboard · Můj život na jedné obrazovce. · Moje finance / 44.5 · data-assistant-530 · MARKET COCKPIT 53.4 · Rozhodnutí 53.4 · MARKET HOME 58.8 · COMMANDER HOME LOOP 59.9 · PERSONAL COMMAND CENTER 60.0