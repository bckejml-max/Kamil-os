...
async function start(){
 ...
 globalThis.__KAMIL_DATA_INTEGRITY1130__={version:'1130.0.0',recoveryKey:RECOVERY_KEY,validate:()=>validateState(store.get()),snapshot:()=>({settledTracked:settledBaseline.size,recovery:!!localStorage.getItem(RECOVERY_KEY)}),stop:()=>{stop();stop=()=>{};started=false}};
}
window.addEventListener('kamil:boot-budget343',()=>void start(),{once:true});
setTimeout(()=>void start(),3500);
