...
function xtbRows(s){
 ...
 return board.map(({p,d})=>{const plan=planByTicker.get(U(p.ticker)),action=U(d.action||'HOLD'),priority=N(d.priority),confidence=d.confidence===null||d.confidence===undefined?null:clamp(d.confidence);let size='';if(plan?.qty)size=`${Number(plan.qty).toLocaleString('cs-CZ',{maximumFractionDigits:4})} ks`;else if(plan?.amount)size=`cca ${money(plan.amount)}`;const why=[d.reason,d.when,plan?.method].filter(Boolean).join(' · ');return{kind:'XTB',name:p.name||p.ticker,ticker:p.ticker,action,actionLabel:actionLabel(action),priority,confidence,size,why,fx:plan?.fx||'',destination:plan?.destination||null,source:d.source||'AUTO'}}).sort((a,b)=>b.priority-a.priority);
}

function ticketRows(s){
 return A(s.ticketBook?.items).filter(active).map(x=>{const d=ticketDecision(x,s),days=daysTo(dueOf(x)),market=marketPrice(x,d),be=breakEven(x),buy=perTicketBuy(x),roi=buy>0&&market>0?Math.round(((market*(1-feeRate(x))-buy)/buy)*1000)/10:null,action=U(d.action||'HOLD'),priority=Math.max(N(d.priority),days!==null&&days<=3?95:0,market>0&&be>0&&market<be?92:0),price=market||null,priceText=price?`${money(price)} / ks`:'';const why=[d.reason,d.when,days!==null?fmtDays(days):'',be?`break-even ${money(be)} / ks`:'',roi!==null?`ROI po poplatku ${roi}%`:'' ].filter(Boolean).join(' · ');return{kind:'Vstupenky',id:x.id,name:x.name||'Vstupenka',action,actionLabel:actionLabel(action),priority,confidence:d.confidence===null||d.confidence===undefined?null:clamp(d.confidence),size:`${Math.max(1,N(x.qty)||1)} ks`,price,priceText,breakEven:be,roi,days,why,source:d.source||'AUTO'}}).sort((a,b)=>b.priority-a.priority);
}

function recommendation(rows){const actionable=rows.filter(x=>!['HOLD','WAIT','REVIEW'].includes(x.action)),urgent=rows.filter(x=>x.priority>=85);if(urgent.length)return `Teď řešit ${urgent.length} urgentní ${urgent.length===1?'rozhodnutí':'rozhodnutí'}.`;if(actionable.length)return `Máš ${actionable.length} konkrétní ${actionable.length===1?'návrh':'návrhy'} k prověření.`;return 'Nevyrábět obchod jen proto, aby se něco dělo.'}

export function marketDecision534(s=store.get()){
 const started=performance.now(),xtb=xtbRows(s),tickets=ticketRows(s),planner=xtbTradePlanner(s),cockpit=ticketSellCockpit(s),all=[...xtb,...tickets].sort((a,b)=>b.priority-a.priority),top=all.slice(0,5),result={xtb,tickets,top,planner,cockpit,recommendation:recommendation(all),urgent:all.filter(x=>x.priority>=85).length,generatedAt:new Date().toISOString()},ms=Math.round((performance.now()-started)*10)/10;window.__KAMIL_DECISION_534_LAST__={ms,at:Date.now(),urgent:result.urgent};return result;
}
...
export async function openMarketDecision534(){
 ...
 const choice=await modal('XTB + vstupenky / Rozhodnutí 53.4',body,[{label:'Execution Readiness 55.5',value:'readiness',primary:true},{label:'Market Suite 55.4',value:'suite'},{label:'Otevřít XTB',value:'money'},{label:'Otevřít vstupenky',value:'tickets'},{label:'Zavřít',value:null}]);
 if(choice==='readiness'){const m=await import('./executionReadiness555.js');return m.openExecutionReadiness555()}
