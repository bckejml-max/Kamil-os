...
function bind(){qsa('[data-more26]',qs('#moreView')).forEach(b=>b.onclick=()=>{mode=b.dataset.more26;renderMore();window.dispatchEvent(new CustomEvent('kamil:more',{detail:mode}))});const backBtn=qs('#more26Back');if(backBtn)backBtn.onclick=()=>{mode='menu';renderMore()};if(mode==='import')bindSmartImport(()=>renderMore());const exportBtn=qs('#backup26Export');if(exportBtn)exportBtn.onclick=()=>{exportBackup();toast('Bezpečná JSON záloha vytvořena');renderMore()};const importInput=qs('#backup26Import');if(importInput)importInput.onchange=e=>{const f=e.target.files?.[0];if(f)importBackup(f)};const maskBtn=qs('#mask26Toggle');if(maskBtn)maskBtn.onclick=()=>store.mutate('Změněn citlivý režim',s=>{s.personalSettings=s.personalSettings||{};s.personalSettings.maskSensitive=!(s.personalSettings.maskSensitive!==false)});const notifyBtn=qs('#notify26Enable');if(notifyBtn)notifyBtn.onclick=async()=>{if('Notification'in window){const p=await Notification.requestPermission();toast(p==='granted'?'Notifikace povoleny':'Notifikace nebyly povoleny');renderMore()}};const cloudBtn=qs('#cloud26Connect');if(cloudBtn)cloudBtn.onclick=()=>window.dispatchEvent(new CustomEvent('kamil:cloud-login'))}
