...
const rerenderHome=async()=>{const m=await import('./homePage140.js');return m.renderHomePage140?.()};
...
export function renderPersonalHome640(){
 ...
 <section class="card home-section" data-home-section="timeline"><div class="eyebrow">DALŠÍCH 12 MĚSÍCŮ</div>${timeline.length?timeline.map(timelineRow).join(''):'<div class="empty success-empty">Nemám známý termín kolem domu v dalších 12 měsících.</div>'}</section>
 ...
 <section class="card home-section" data-home-section="maintenance"><div class="eyebrow">ÚDRŽBA A SERVIS</div>${maintenance.length?maintenance.map(({item:x},i)=>`<div class="row ux64-row"><div><b>${h(x.title||x.name||'Údržba')}</b><div class="muted">${h(x.due||x.nextAt||x.deadline||'bez termínu')}</div></div><button class="btn" data-maintenance="${i}">Řešit</button></div>`).join(''):'<div class="empty success-empty">Teď není evidovaná žádná údržba k řešení.</div>'}</section></div>`;
 host.querySelectorAll('[data-home-record]').forEach(b=>{b.onclick=async()=>{const x=home[Number(b.dataset.homeRecord)];await editHomeRecord644(x.id);await rerenderHome()}});
 host.querySelectorAll('[data-maintenance]').forEach(b=>{b.onclick=async()=>{const x=maintenance[Number(b.dataset.maintenance)];await openMaintenance644(x.item,x.source);await rerenderHome()}});
