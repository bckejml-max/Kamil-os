...
export function renderPersonalHome640(){
 ...
 host.innerHTML=`<div class="ux64-page"><div class="view-head"><div><div class="eyebrow">DOMOV</div><h1>Co bude dům potřebovat</h1><p>Smlouvy, energie, servis a údržba v jednom praktickém plánu.</p></div></div>
 <section class="card"><div class="eyebrow">DALŠÍCH 12 MĚSÍCŮ</div>${timeline.length?timeline.slice(0,10).map(timelineRow).join(''):'<div class="empty success-empty">Nemám známý termín kolem domu v dalších 12 měsících.</div>'}</section>
 <section class="ux64-contract-grid">${home.map(card).join('')||'<div class="card">Zatím bez uložených údajů o domovu.</div>'}</section>
 <section class="card"><div class="eyebrow">ÚDRŽBA A SERVIS</div>${maintenance.length?maintenance.slice(0,8).map(({item:x},i)=>`<div class="row ux64-row"><div><b>${h(x.title||x.name||'Údržba')}</b><div class="muted">${h(x.due||x.nextAt||x.deadline||'bez termínu')}</div></div><button class="btn" data-maintenance="${i}">Řešit</button></div>`).join(''):'<div class="empty success-empty">Teď není evidovaná žádná údržba k řešení.</div>'}</section></div>`;
 ...
 if(typeof window!=='undefined')window.__KAMIL_PERSONAL_HOME_650_LAST__={at:Date.now(),records:home.length,maintenance:maintenance.length,timeline:timeline.length};
}
