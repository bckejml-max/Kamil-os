...
export function mission438(s=store.get()){
 ...
 const seen=new Set(),top=candidates.sort((a,b)=>b.score-a.score).filter(x=>{const k=`${x.kind}|${x.title}`;if(seen.has(k))return false;seen.add(k);return true}).slice(0,3);
 return {top,finance,work,status:top.some(x=>x.score>=105)?'ZÁSAH':top.length?'ŘÍDIT':'KLID'};
}
