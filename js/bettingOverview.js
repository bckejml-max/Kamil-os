...
function data(){
 const l=ledger(),open=l.bets.filter(x=>String(x.status||'OPEN').toUpperCase()==='OPEN').sort((a,b)=>Number(b.stakeCzk||0)-Number(a.stakeCzk||0)),settled=l.bets.filter(x=>['WIN','LOSS','VOID'].includes(String(x.status||'').toUpperCase()));
...
export function renderBettingOverview(){
 ...
 const d=data(),hasBankroll=d.bankrollCzk>0,hasHistory=d.settled.length>0,risk=hasBankroll&&d.exposure>d.bankrollCzk*.2,unknownRisk=d.exposure>0&&!hasBankroll,largest=d.open[0]||null,riskPct=hasBankroll?d.exposure/d.bankrollCzk*100:null;
 ...
 '<div class="pr1300-head"><div><div class="pr1300-kicker">Sázení</div><h1>Peníze ve hře a riziko.</h1><p>Všechny otevřené sázky jsou vidět přímo. Scanner, modely a historie zůstávají až v detailu.</p></div><span class="pr1300-status '+statusTone+'">'+d.open.length+' otevřených</span></div>' +
 ...
 '<div class="pr1320-meta os1334-history-meta"><span>Profit: '+(hasHistory?money(d.profit):'—')+'</span><span>ROI: '+(hasHistory?d.roi.toFixed(1).replace('.',',')+' %':'—')+'</span><span>Win rate: '+(hasHistory?d.winRate.toFixed(1).replace('.',',')+' %':'—')+'</span></div>' +
 ...
 window.__KAMIL_BETTING_OVERVIEW__={healthy:true,open:d.open.length,renderedOpen:d.open.length,exposure:d.exposure,profit:d.profit,hasBankroll,hasHistory,riskUnknown:unknownRisk,decisionSurface:1334,at:Date.now()};return true;
}
