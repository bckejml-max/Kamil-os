...
export function renderBettingOverview(){
 const host=document.querySelector('#bettingView');if(!host)return false;if(host.dataset.productAdvanced==='1')return true;const d=data(),hasBankroll=d.bankrollCzk>0,hasHistory=d.settled.length>0,risk=hasBankroll&&d.exposure>d.bankrollCzk*.2,unknownRisk=d.exposure>0&&!hasBankroll;
 ...
 '<div class="pr1300-head"><div><div class="pr1300-kicker">Sázení</div><h1>Otevřené sázky a riziko.</h1><p>Scanner, modely a historie jsou detail. Tady jsou jen peníze, které jsou právě ve hře.</p></div><span class="pr1300-status '+statusTone+'">'+d.open.length+' otevřených</span></div>' +
 '<section class="pr1320-now"><div><div class="pr1300-kicker">Expozice</div><h2>'+money(d.exposure)+'</h2><p>'+riskCopy+'</p></div><div class="pr1320-now-actions"><button class="pr1300-btn primary" type="button" data-betting-advanced>Scanner a detail sázek</button></div></section>' +
 '<div class="pr1320-meta"><span>Bankroll: '+(hasBankroll?money(d.bankrollCzk):'nenastaven')+'</span><span>Profit: '+(hasHistory?money(d.profit):'—')+'</span><span>ROI: '+(hasHistory?d.roi.toFixed(1).replace('.',',')+' %':'—')+'</span><span>Win rate: '+(hasHistory?d.winRate.toFixed(1).replace('.',',')+' %':'—')+'</span></div>' +
 (d.open.length?'<section class="pr1300-panel"><div class="pr1300-panel-head"><h2>Otevřené sázky</h2><span>jen aktivní expozice</span></div>'+rows(d.open)+'</section>':'<section class="pr1300-panel pr1330-betting-clear"><div class="pr1300-panel-head"><h2>Bez otevřených sázek</h2><span>expozice 0 Kč</span></div><div class="pr1300-empty">Teď není nic ve hře. Historii a scanner otevřeš jen pokud je potřebuješ.</div></section>') +
 ...
 window.__KAMIL_BETTING_OVERVIEW__={healthy:true,open:d.open.length,exposure:d.exposure,profit:d.profit,hasBankroll,hasHistory,riskUnknown:unknownRisk,at:Date.now()};return true;
}