...
import {dailyAutopilot43} from './platform43.js';
...
function check(){if(!canNotify())return;const a=dailyAutopilot43(store.get());if(a.top?.length)notifyDesktop43('Kamil OS · dnešní priority',a.top.map((x,i)=>`${i+1}. ${x.title}`).join(' · '),{key:'daily-priorities'})}
setTimeout(check,22000);window.addEventListener('focus',()=>setTimeout(check,1200));
