...
export async function upsertTicketInventory660(rows,{syncLocal:doLocal=true}={}){
 const {c,sess}=await ctx();if(!c||!sess)return{ok:false,reason:'NO_SESSION'};const payload=cloudPayload(rows,sess.user.id);const {error}=await c.from('ticket_inventory').upsert(payload,{onConflict:'user_id,id'});if(error)return{ok:false,error};if(doLocal)syncLocal(rows);return{ok:true,count:payload.length};
}
export async function replaceTicketInventory660(rows){
 const {c,sess}=await ctx();if(!c||!sess)return{ok:false,reason:'NO_SESSION'};const current=await c.from('ticket_inventory').select('id,viagogo_url,ask_each_czk').eq('user_id',sess.user.id);if(current.error)return{ok:false,error:current.error};const old=new Map((current.data||[]).map(x=>[x.id,x])),merged=rows.map(x=>({...x,viagogoUrl:x.viagogoUrl||old.get(x.id)?.viagogo_url||null,askEachCzk:Number(x.askEachCzk||old.get(x.id)?.ask_each_czk||0)||null})),payload=cloudPayload(merged,sess.user.id);const {error}=await c.from('ticket_inventory').upsert(payload,{onConflict:'user_id,id'});if(error)return{ok:false,error};const keep=new Set(payload.map(x=>x.id)),stale=(current.data||[]).map(x=>x.id).filter(id=>!keep.has(id));if(stale.length){const del=await c.from('ticket_inventory').delete().eq('user_id',sess.user.id).in('id',stale);if(del.error)return{ok:false,error:del.error}}syncLocal(merged);return{ok:true,count:payload.length,removed:stale.length};
}
