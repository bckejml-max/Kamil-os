...
async function openResult(result){
 if(!result)return null;markPersonalUsage650('action','search-result');
 ...
 if(result.route==='family')return nav('family');if(result.route==='documents')return nav('more');if(['today','home','money'].includes(result.route))return nav(result.route);return nav('today')
}
