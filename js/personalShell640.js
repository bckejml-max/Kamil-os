...
function apply(view='today'){
 currentView=view;const page=qs('#pageTitle');if(page)page.textContent=TITLES[view]||'DNES';document.title='Kamil OS';
...
async function openResult(result){if(!result)return null;if(result.type==='data')return openVaultRecord640(result.id);if(result.type==='waiting')return openPersonalWaiting650();if(result.route==='family')return nav('tickets');if(result.route==='documents')return nav('more');if(['today','home','money'].includes(result.route))return nav(result.route);return nav('today')}
async function askGlobal(){
 const input=qs('#commandInput'),q=input?.value.trim();if(!q){input?.focus();return null}const a=answerPersonalQuestion640(q);if(input)input.value='';
...
export function bindPersonalShell640(){
 if(bound)return;bound=true;qsa('[data-personal-more]').forEach(b=>b.addEventListener('click',()=>openPersonalMore640()));window.addEventListener('kamil:view-change',e=>apply(e.detail));
 ...
 const add=qs('#quickAddBtn');if(add)add.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();openPersonalCapture643(captureType())},true);apply('today');
}
