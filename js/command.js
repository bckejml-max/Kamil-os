...
export function parse(raw){
 ...
 m=n.match(/^(.+?)\s+z[ií]tra$/);if(m)return{type:'tomorrow',name:m[1]};
...
export function execute(raw){
 ...
 if(c.type==='tomorrow'){
   const x=S().tasks.find(t=>t.status!=='HOTOVO'&&norm(t.title).includes(norm(c.name)));if(!x)return toast('Úkol jsem nenašel.');
   once(`tomorrow|${x.id}`,()=>store.mutate('Úkol přesunut na zítra',s=>{const t=s.tasks.find(y=>y.id===x.id),d=new Date();d.setDate(d.getDate()+1);d.setHours(9,0,0,0);t.due=d.toISOString();t.updatedAt=new Date().toISOString()}));return
 }
