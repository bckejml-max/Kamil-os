...
export const personalDailyMode652=(now=new Date())=>{const hour=now.getHours();return hour<11?'morning':hour>=21?'late':hour>=18?'evening':'day'};

export function personalDailyRhythm651(s=store.get()){
 const d=personalDailyAssistant650(s),w=personalWaitingCenter650(s),done=doneToday(s),mode=personalDailyMode652();
 ...
 const summary=mode==='late'
  ?(urgent.length?`Je pozdě. Pokud chceš ještě něco řešit, stačí ${urgent.length===1?'tahle jedna důležitá věc':`${urgent.length} opravdu důležité věci`}.`:'Dnešek můžeš uzavřít. Nic osobního už nehoří.')
  :mode==='evening'
 ...
 return{mode,done,urgent,tomorrow,followups,summary,canClose:mode==='late'&&urgent.length===0};
}
...
export async function openDailyClose651(){
 ...
 return modal(x.mode==='late'?'Uzavření dne':'Denní přehled',body,[{label:x.canClose?'Dnešek uzavřen':'Hotovo',value:null,primary:true}]);
}
