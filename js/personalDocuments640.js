...
export async function openVaultRecord640(id){
 ...
 const choice=await modal('Dokument a data',body,actions);if(choice==='confirm'){confirmVaultRecord640(id);toast('Údaj potvrzen a uložen mezi osobní data.');return true}return choice;
}
...
export function renderPersonalDocuments640(){
 ...
 host.innerHTML=`<div class="ux64-page"><div class="view-head"><div><div class="eyebrow">DOKUMENTY A DATA</div><h1>Smlouvy, pojistky a důležité údaje</h1><p>Ověřování běží pod kapotou. Tady vidíš jen stav, zdroj a další krok.</p></div></div><div class="metric-strip"><div class="metric"><span>Pokrytí osobních dat</span><b>${v.coverage}%</b></div><div class="metric"><span>K ověření / aktualizaci</span><b>${v.action.length}</b></div><div class="metric"><span>Uložených záznamů</span><b>${v.records.length}</b></div><div class="metric"><span>Cloud</span><b>${s.meta?.cloudMode==='cloud'?'Připojen':'Lokálně'}</b></div></div>
 <section class="ux64-doc-list">${records.map(row).join('')}</section><div class="decision-note">Potvrzení se ukládá do historie ověření i mezi osobní data. Pokud máš připojený cloud, synchronizuje se spolu s ostatními osobními údaji.</div></div>`;
