...
const row=v=>`<article class="ux64-doc ux65-doc"><div class="ux64-doc-main"><div class="ux64-contract-head"><div><span class="ux64-type">${typeLabel(v)}</span><h2>${h(v.title)}</h2></div><span class="ux64-status">${h(v.status.label)}</span></div><div class="muted">${h(validityLabel(v))}</div><div class="ux64-next"><b>Co dál:</b> ${h(v.nextAction)}</div></div><button class="btn ${v.status.severity?'primary':''}" data-vault-record="${h(v.id)}">${v.status.severity?'Vyřešit':'Otevřít'}</button></article>`;
...
export function renderPersonalDocuments640(){
 ...
 host.innerHTML=`<div class="ux64-page"><div class="view-head"><div><div class="eyebrow">DOKUMENTY</div><h1>Co je potřeba hlídat</h1><p>Smlouvy, pojistky a důležité údaje. Na kartě jen stav, termín a další krok.</p></div><div class="row-actions"><button class="btn primary" id="documentInbox650">+ Přidat dokument / zdroj</button></div></div>
 ${v.action.length?`<div class="decision-note"><b>${v.action.length} ${v.action.length===1?'věc nemá':'věci nemají'} aktuální potvrzení.</b> Nejdřív řeš karty označené tlačítkem Vyřešit.</div>`:'<div class="decision-note good"><b>Základní dokumentová data jsou aktuálně v pořádku.</b></div>'}
