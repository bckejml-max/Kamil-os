...
export function renderToday(){
 ...
 qs('#todayView').innerHTML=`
  ...
  <div class="grid" style="margin-top:2px">
    ...
    <div class="card">
      ...
      ${projects.map(p=>`<div class="row"><div><b>${h(p.name)}</b><div class="muted">${p.owner?h(p.owner)+' · ':''}${h(p.next||'Chybí další krok')}</div></div><button class="btn" data-today-project="${p.id}">Otevřít</button></div>`).join('')||'<div class="empty">Žádný aktivní projekt.</div>'}
      <button class="btn" data-nav="work" style="margin-top:8px">Všechny projekty</button>
    </div>
  </div>

  <div class="grid two" style="margin-top:2px">
    <div class="card"><div class="eyebrow">ČEKÁM NA</div>${waiting.map(x=>`<div class="row"><div><b>${h(x.title||x.person||'Čekající položka')}</b><div class="muted">${x.person?h(x.person)+' · ':''}${waitAge(x)} dní čekání</div></div><button class="btn" data-wait-done="${x.id}">Vyřešeno</button></div>`).join('')||'<div class="empty">Na nikoho kriticky nečekáš.</div>'}${waitingAll.length>waiting.length?`<div class="muted" style="padding-top:8px">+ ${waitingAll.length-waiting.length} dalších čekajících položek</div>`:''}<button class="btn" data-more-nav="waiting" style="margin-top:10px">Otevřít všechno čekání · ${waitingAll.length}</button></div>
    <div class="card"><div class="eyebrow">RADAR</div>${nextSignals.map((x,i)=>`<div class="row"><div><b>${h(x.title)}</b><div class="muted">${h(x.type)} · ${h(x.reason)}</div></div><div class="row-actions"><span class="status ${x.score>=85?'bad':x.score>=65?'warn':'good'}">${x.score}</span><button class="btn" data-radar-open="${i}">Otevřít</button></div></div>`).join('')||'<div class="empty">Žádný další výrazný signál.</div>'}${inbox?`<div class="row"><div><b>Inbox</b><div class="muted">${inbox} položek čeká na rozhodnutí</div></div><button class="btn" data-more-nav="inbox">Projít</button></div>`:''}</div>
  </div>
 `;

 qs('#focusOpen').onclick=()=>openSignal(rec);
 ...
 qsa('[data-today-project]',qs('#todayView')).forEach(b=>b.onclick=()=>openProject(b.dataset.todayProject));
 qsa('[data-radar-open]',qs('#todayView')).forEach(b=>b.onclick=()=>openSignal(nextSignals[Number(b.dataset.radarOpen)]));
 qsa('[data-wait-done]',qs('#todayView')).forEach(b=>b.onclick=()=>store.mutate('Čekání vyřešeno',x=>{const w=x.delegations?.find(y=>y.id===b.dataset.waitDone);if(w){w.status='DONE';w.doneAt=new Date().toISOString();w.updatedAt=new Date().toISOString()}}));
