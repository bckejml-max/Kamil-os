...
const dateMs=v=>{const t=new Date(v||0).getTime();return Number.isFinite(t)&&t>0?t:null};
...
export function dataQuality(s={},meta={},now=new Date()){
 ...
 const bh=backupHealth(s,meta,now);if(bh.status==='NO_BACKUP')add('Záloha','Chybí přenosná záloha','Ještě není evidovaný export Backup Guard.',86,'more',null,'backup');else if(bh.status==='STALE')add('Záloha','Záloha je starší než 30 dní',`Poslední export: ${bh.lastBackupAt||'neznámý'}.`,74,'more',null,'backup');
 issues.sort((a,b)=>b.priority-a.priority||String(a.title).localeCompare(String(b.title),'cs'));
 return {issues,total:issues.length,high:issues.filter(x=>x.priority>=75).length,medium:issues.filter(x=>x.priority>=60&&x.priority<75).length,score:Math.max(0,100-Math.min(100,issues.filter(x=>x.priority>=75).length*14+issues.filter(x=>x.priority>=60&&x.priority<75).length*5)),top:issues.slice(0,8),note:'Data Quality hodnotí pouze úplnost uložených údajů. Chybějící údaj neznamená, že daný závazek skutečně existuje nebo je problém.'};
}
...
export function familyProfiles(s={},now=new Date()){
 ...
 return members.map(m=>{
  ...
  const docs=linked.filter(x=>x.category==='DOCUMENT'),insurance=linked.filter(x=>x.category==='INSURANCE'),obligations=linked.filter(x=>x.category==='FAMILY');
  return {...m,documents:docs,insurance,obligations,linkedCount:linked.length,upcoming:[m.birthday&&{kind:'Narozeniny',at:m.birthday},m.anniversary&&{kind:'Výročí',at:m.anniversary}].filter(Boolean).map(x=>({...x,days:dayDiff(x.at,now)})).filter(x=>x.days!==null).sort((a,b)=>a.days-b.days)};
 });
}
...
export function notificationQueue(s={},meta={},now=new Date()){
 ...
 return {items:out.slice(0,12),critical:out.filter(x=>x.priority>=90).length,important:out.filter(x=>x.priority>=80).length,top:out[0]||null,note:'Browserové upozornění může Kamil OS zobrazit jen při povolení prohlížečem a když je aplikace spuštěná. Skutečný background push vyžaduje serverovou push službu.'};
}

export function smartBriefing(s={},meta={},now=new Date()){
 const base=personalBriefing(s,now),notifications=notificationQueue(s,meta,now),quality=dataQuality(s,meta,now),inbox=personalInbox(s,now);
...
const fmtCurrencyMap=map=>Object.entries(map||{}).map(([c,v])=>`${c}: ${Math.round(Number(v||0)).toLocaleString('cs-CZ')}`).join(' · ')||'bez známé částky';
export function personalQuery(raw,s={},meta={},now=new Date()){
 const q=norm(raw);if(!q)return null;
 const tl=personalTimeline(s,now),pm=personalMoney(s,now),quality=dataQuality(s,meta,now),brief=smartBriefing(s,meta,now),assets=assetBook(s,now),alerts=notificationQueue(s,meta,now);
 let m=q.match(/(?:konci|expiruje|termin).*?(\d{1,3})\s*(?:dni|dnu|d)/);if(m){const days=Math.min(365,Math.max(1,Number(m[1]))),rows=tl.items.filter(x=>x.days>=0&&x.days<=days).slice(0,12);return {title:`Co končí / čeká do ${days} dní`,lines:rows.map(x=>`${x.title} — ${x.days===0?'dnes':`za ${x.days} dní`} (${x.domain})`),note:rows.length?'Jen z uložených osobních termínů.':'V uložených osobních datech nic takového není.'}}
 ...
 if(q.includes('auto')){const a=assets.items.filter(x=>x.kind==='VEHICLE'),admin=(s.personalAdmin?.items||[]).filter(x=>active(x)&&(x.category==='VEHICLE'||x.insurance?.kind==='VEHICLE'||['STK','VIGNETTE','SERVICE'].includes(x.document?.kind)));return {title:'Všechno kolem auta',lines:[...a.map(x=>`${x.title} — ${x.reason}`),...admin.map(x=>`${x.title} — ${x.provider||x.category||'evidence'}`)].slice(0,15),note:'Majetek + osobní administrativní položky související s autem.'}}
 if(q.includes('dum')||q.includes('domacnost')){const a=assets.items.filter(x=>['HOME_SYSTEM','APPLIANCE','PROPERTY'].includes(x.kind)),admin=(s.personalAdmin?.items||[]).filter(x=>active(x)&&['HOME','UTILITY'].includes(x.category));return {title:'Dům a domácnost',lines:[...a.map(x=>`${x.title} — ${x.reason}`),...admin.map(x=>`${x.title} — ${x.provider||x.category||'evidence'}`)].slice(0,15),note:'Pouze uložené domácí položky a majetek.'}}
 if((q.includes('stat penize')||q.includes('stoji penize')||q.includes('stat penize'))&&q.includes('mesic')){const rows=alerts.items.filter(x=>['Platby','Smlouvy','Riziko','Majetek'].includes(x.domain)).slice(0,10);return {title:'Co může stát peníze tento měsíc',lines:rows.map(x=>`${x.title} — ${x.detail}`),note:'Pravidlový přehled z uložených termínů a rizik; není to předpověď neznámých výdajů.'}}
 if(q.includes('co chybi')||q.includes('doplnit data')||q.includes('data quality'))return {title:'Co chybí doplnit',lines:quality.top.map(x=>`${x.title} — ${x.detail}`),note:quality.note};
 if(q.includes('co mam dnes')||q.includes('dnes resit')||q.includes('co resit'))return {title:'Co řešit dnes',lines:brief.today.map(x=>`${x.title} — ${x.reason||'řešit'}`),note:brief.today.length?brief.note:'Nic zásadního dnes nevychází.'};
 if(q.includes('kolik stoji zivot')||q.includes('mesicni naklady')||q.includes('mesicne platim'))return {title:'Známé měsíční náklady',lines:Object.entries(pm.byCurrency||{}).map(([c,v])=>`${Math.round(v.monthly||0).toLocaleString('cs-CZ')} ${c} / měsíc`),note:pm.note};
...
export function autopilotSnapshot(s={},meta={},now=new Date()){
 return {briefing:smartBriefing(s,meta,now),notifications:notificationQueue(s,meta,now),quality:dataQuality(s,meta,now),inbox:personalInbox(s,now),money:householdCockpit(s,now),assets:assetBook(s,now),family:familyProfiles(s,now),note:autopilotNote};
}
