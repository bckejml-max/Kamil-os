...
function localCurrencyRisk(rows,currency,targets,s){const total=rows.reduce((z,x)=>z+n(x.value),0),prepared=rows.map(x=>({...x,value:n(x.value),bucket:classifyRebalancePosition(x,s)})),buckets=bucketSummary(prepared,total,targets),con=concentration(prepared,total);return {currency,total:round1(total),positions:con.positions,top1Pct:con.top1Pct,top3Pct:con.top3Pct,effectivePositions:con.effectivePositions,buckets}}
export function portfolioRiskMap(s={},opts={}){
 ...
 const local=Object.fromEntries(Object.entries(byCurrency).map(([currency,rows])=>[currency,localCurrencyRisk(rows,currency,targets,s)]));
