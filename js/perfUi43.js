...
function diagnostics(){
 const storage=coldStorageStats42(),nav=performance.getEntriesByType?.('navigation')?.[0]||null,long=performance.getEntriesByType?.('longtask')||[];
 ...
 return {storage,nav,long,shell,app,today,bottleneck};
}
function tile(){
...
async function openPerf43(){
 const d=diagnostics(),ttfb=d.nav?Math.max(0,d.nav.responseStart-d.nav.requestStart):null,dom=d.nav?.domContentLoadedEventEnd||null;
 const body=`<div class="decision-note"><b>${h(d.bottleneck)}</b><br><span class="muted">Měření je lokální na tomto zařízení. Nic se neposílá ven.</span></div><div class="metric-strip"><div class="metric"><span>Instant shell</span><b>${h(ms(d.shell))}</b></div><div class="metric"><span>App ready</span><b>${h(ms(d.app))}</b></div><div class="metric"><span>Plný Today</span><b>${h(ms(d.today))}</b></div><div class="metric"><span>TTFB</span><b>${h(ms(ttfb))}</b></div></div><div class="card"><div class="row"><span>Horký lokální stav</span><b>${h(kb(d.storage.mainBytes))}</b></div><div class="row"><span>Cold historie</span><b>${h(kb(d.storage.coldBytes))}</b></div><div class="row"><span>Sync fronta</span><b>${h(kb(d.storage.queueBytes))}</b></div><div class="row"><span>DOMContentLoaded</span><b>${h(ms(dom))}</b></div><div class="row"><span>Dlouhé tasky dostupné v timeline</span><b>${d.long.length}</b></div><div class="row"><span>Načtené cold domény</span><b>${h((d.storage.hydrated||[]).join(', ')||'žádné')}</b></div></div><p class="muted">Optimalizace pouze přesune historická pole do lazy úložiště. Záznamy nemaže.</p>`;
