...
import {openTicketBuyRadar674,bindTicketBuyRadar674} from './ticketBuyRadar674.js';
...
function loadingHtml(){return `<div class="ux64-page"><div class="view-head"><div><div class="eyebrow">VSTUPENKY 67.4</div><h1>Ticket Decision Center</h1><p>Načítám portfolio a poslední tržní snapshoty…</p></div></div><section class="card"><div class="empty">Načítám Ticket Intelligence…</div></section></div>`}

function errorHtml(reason='Ticket cloud není dostupný.'){
 return `<div class="ux64-page"><div class="view-head"><div><div class="eyebrow">VSTUPENKY 67.4</div><h1>Ticket Decision Center</h1><p>${h(reason)}</p></div><div class="row-actions"><button class="btn primary" id="ticketOpenCommander671">Otevřít Ticket Commander</button><button class="btn" id="ticketBuyRadar674">Buy Radar</button></div></div><section class="card"><div class="empty">Pro Decision Engine potřebuji připojený ticket cloud. Původní Ticket Commander zůstává dostupný.</div></section></div>`;
}
...
function viewHtml(cloud){
 ...
 return `<div class="ux64-page"><div class="view-head"><div><div class="eyebrow">VSTUPENKY 67.4</div><h1>Ticket Decision Center</h1><p>${h(statusText(cloud))}</p></div><div class="row-actions"><button class="btn primary" id="ticketRefresh671">Obnovit trh</button><button class="btn" id="ticketBuyRadar674">Buy Radar</button><button class="btn" id="ticketOpenCommander671">Detailní Commander</button></div></div>${ticketDecisionSummaryHtml671(cloud)}<div class="view-head compact"><div><div class="eyebrow">PORTFOLIO</div><h2>Rozhodnutí podle pozic</h2></div></div>${decisions||'<section class="card"><div class="empty success-empty">Žádná aktivní ticketová pozice.</div></section>'}${rows.length>8?`<section class="card"><div class="muted">Zobrazuji 8 prioritních pozic. Kompletní portfolio otevři v Detailním Commanderu.</div></section>`:''}</div>`;
}

function bind(host){
 bindTicketBuyRadar674();
 host.querySelector('#ticketOpenCommander671')?.addEventListener('click',()=>openTicketCommander660());
 host.querySelector('#ticketBuyRadar674')?.addEventListener('click',()=>openTicketBuyRadar674());
...
async function hydrate(host,token){
 ...
 if(typeof window!=='undefined')window.__KAMIL_TICKETS_674_LAST__={at:Date.now(),active:cloud.inventory.filter(active).length,snapshots:cloud.snapshots.length,alerts:cloud.alerts.length,buyRadar:true};
}
