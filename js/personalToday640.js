...
import {appendDailyTicketPriority672} from './personalTicketPriority672.js';
...
export function renderPersonalToday640(){
 ...
 const late=rhythm.mode==='late',morning=rhythm.mode==='morning',primary=late?(rhythm.urgent[0]||null):d.primary,secondary=late?rhythm.urgent.slice(1,3):d.secondary;
 const headline=late?rhythm.summary:d.headline;
 const quick=late?`<section class="ux65-quick"><button class="btn primary" data-daily-close>Uzavřít den</button></section>`:morning?`<section class="ux65-quick"><button class="btn primary" data-morning-open>Ranní přehled</button></section>`:`<section class="ux65-quick"><button class="btn" data-ask="Co mám dnes řešit?">Co dnes řešit?</button><button class="btn" data-ask="Co mi končí?">Co mi končí?</button><button class="btn" data-ask="Na co čekám?">Na co čekám?</button><button class="btn" data-daily-close>${rhythm.mode==='evening'?'Uzavřít den':'Denní přehled'}</button></section>`;
 ...
 appendDailyTicketPriority672(host).then(x=>{if(typeof window!=='undefined'&&window.__KAMIL_PERSONAL_UX_672_LAST__)window.__KAMIL_PERSONAL_UX_672_LAST__.ticketPriority=x?.title||null}).catch(()=>null);
 if(typeof window!=='undefined')window.__KAMIL_PERSONAL_UX_672_LAST__={at:Date.now(),view:'today',primary:primary?.title||null,secondary:secondary.map(x=>x.title),waiting:d.waitingCount,tomorrow:d.tomorrowCount,next7:d.next7Count,doneToday:rhythm.done,mode:rhythm.mode,lateCalm:late&&!primary,nightHandoff:late,morningLaunch:morning,morningCalendar:morningData.calendar.length,morningFollowups:morningData.followups.length,ticketPriority:'loading'};
}
