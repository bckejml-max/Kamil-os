...
export function renderPersonalToday640(){
 ...
 const quick=late?`<section class="ux65-quick"><button class="btn" data-tomorrow-open>Otevřít zítřek</button><button class="btn primary" data-daily-close>Uzavřít den</button></section>`:`<section class="ux65-quick"><button class="btn" data-ask="Co mám dnes řešit?">Co dnes řešit?</button><button class="btn" data-ask="Co mi končí?">Co mi končí?</button><button class="btn" data-ask="Na co čekám?">Na co čekám?</button><button class="btn" data-daily-close>${rhythm.mode==='evening'?'Uzavřít den':'Denní přehled'}</button></section>`;
 ...
 ${late?tomorrowPreview(d.tomorrow):''}
 ...
 if(typeof window!=='undefined')window.__KAMIL_PERSONAL_UX_654_LAST__={at:Date.now(),view:'today',primary:primary?.title||null,secondary:secondary.map(x=>x.title),waiting:d.waitingCount,tomorrow:d.tomorrowCount,next7:d.next7Count,doneToday:rhythm.done,mode:rhythm.mode,lateCalm:late&&!primary,nightHandoff:late};
}
