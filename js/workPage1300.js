...
function render(){
 ...
 const m=workCommandCenter440(store.get()),totalExposure=m.finance.changeExposure+m.finance.receivable+m.finance.unbilled,top=m.topRisks[0];
 host.innerHTML='<div class="pr1300-shell" data-work-page1300>' +
  '<div class="pr1300-head"><div><div class="pr1300-kicker">Práce</div><h1>Zakázky a další kroky.</h1><p>Nejdřív termín nebo riziko, které vyžaduje zásah. Teprve potom přehled aktivních zakázek.</p></div><span class="pr1300-status '+(m.status==='ZÁSAH'?'bad':m.status==='SLEDOVAT'?'warn':'good')+'">'+esc(m.status)+'</span></div>' +
  '<section class="pr1320-now"><div><div class="pr1300-kicker">Teď</div><h2>'+esc(top?.title||'Nic kritického')+'</h2><p>'+esc(top?.detail||'Podle uložených dat teď žádná zakázka ani pravidelný termín nevyžaduje okamžitý zásah.')+'</p></div><div class="pr1320-now-actions"><button class="pr1300-btn primary" type="button" data-work1300-add>＋ Pracovní úkol</button></div></section>' +
