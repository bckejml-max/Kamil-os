...
export function todayBrain34(state={},now=new Date()){
 const out=[],director=directorBriefing34(state,now),tickets=ticketMarketBrain34(state,now),portfolio=totalInvestmentPortfolio34(state),budget=Number(state.financePlan?.plannedInvestment||0),plan=monthlyInvestmentPlan34(state,budget||25000);
 ...
 return {rows,top:rows[0]||null,critical:rows.filter(x=>x.priority>=90).length,director,tickets,portfolio,plan,note:'Jeden společný žebříček přes práci, vstupenky, investice a osobní termíny. Priorita neznamená automatické provedení; jen pořadí, co má největší smysl řešit.'};
}
