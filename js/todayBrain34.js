...
export function todayBrain34(state={},now=new Date()){
 const out=[],director=directorBriefing34(state,now),tickets=ticketMarketBrain34(state,now),waiting=waitingFor35(state,now),email=emailWorkflow35(state,now),portfolio=totalInvestmentPortfolio34(state),budget=Number(state.financePlan?.plannedInvestment||0),plan=monthlyInvestmentPlan34(state,budget||25000);
 for(const x of email.triage.slice(0,3)){if(x.priority<85)continue;out.push(item(`E-mail · ${x.title}`,x.reason,x.priority,'email','E-mail',{id:x.id}))}
 ...
 return {rows,top:rows[0]||null,critical:rows.filter(x=>x.priority>=90).length,director,tickets,waiting,email,portfolio,plan,note:'Jeden společný žebříček přes práci, e-maily, follow-upy, vstupenky, investice a osobní termíny. Priorita neznamená automatické provedení; jen pořadí, co má největší smysl řešit.'};
}
