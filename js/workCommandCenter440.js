...
const CLOSED=/HOTOV|ARCHIV|DONE|CLOSED|RESOLVED|PAID|SOLD|CANCELLED|CANCELED|COMPLETED|FINISHED/;
const CLOSED_EXACT=new Set(['PŘEDÁNO','PŘEDANÉ','PREDANO','PREDANE']);
const open=x=>{const s=U(x?.status||x?.workflow).trim();return !CLOSED_EXACT.has(s)&&!CLOSED.test(s)};
