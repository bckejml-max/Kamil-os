...
const OWNER='system.diagnostics421';
let runPromise=null;
...
function renderLoaderStats437(){
  ...
  root.onclick=e=>{const load=e.target.closest?.('#loadCold437');if(load){window.dispatchEvent(new CustomEvent('kamil:load-cold-module',{detail:{path:cold}}));load.disabled=true;load.textContent='Načítám…';return}const heal=e.target.closest?.('[data-self-heal-module]');if(heal){const path=heal.dataset.selfHealModule;if(!path)return;heal.disabled=true;heal.textContent='Testuji…';window.dispatchEvent(new CustomEvent('kamil:self-heal-now',{detail:{path}}));return}const retry=e.target.closest?.('[data-retry-module]');if(retry){const path=retry.dataset.retryModule;if(!path)return;retry.disabled=true;retry.textContent='Načítám…';window.dispatchEvent(new CustomEvent('kamil:retry-quarantined-module',{detail:{path}}));return}if(e.target.closest?.('#resetLoader437')){try{localStorage.removeItem(MODULE_STATS_KEY);window.__KAMIL_MODULE_STATS__={};window.__KAMIL_COLD_MODULE__=null}catch{}window.dispatchEvent(new CustomEvent('kamil:loader-stats-reset'));renderLoaderStats437();return}if(e.target.closest?.('#resetMemory437')){try{localStorage.removeItem(STABILITY_MEMORY_KEY);window.__KAMIL_STABILITY_MEMORY__={};window.__KAMIL_MEMORY_BLOCKED_MODULES__=[];window.__KAMIL_MEMORY_COLD_MODULES__=[]}catch{}window.dispatchEvent(new CustomEvent('kamil:stability-memory-reset'));renderLoaderStats437();return}if(e.target.closest?.('#resetQuarantine437')){try{localStorage.removeItem(QUARANTINE_KEY);window.__KAMIL_QUARANTINED_MODULES__={}}catch{}window.dispatchEvent(new CustomEvent('kamil:quarantine-reset'));renderLoaderStats437()}};
...
export function renderSystemDiagnostics421(){
  ...
  runPromise=Promise.allSettled([
    ...
    schedule1100(OWNER,'render',renderLoaderStats437,80,{pauseWhenHidden:true});
    return true;
  }).finally(()=>{runPromise=null});
  return runPromise;
}

for(const event of ['kamil:cold-module-loaded','kamil:module-quarantined','kamil:module-quarantine-recovered','kamil:module-quarantine-retry-failed','kamil:module-quarantine-cleared','kamil:self-heal-probing','kamil:module-self-healed','kamil:self-heal-failed'])ownEvent1100(OWNER,window,event,()=>schedule1100(OWNER,'render',renderLoaderStats437,80,{pauseWhenHidden:true}));