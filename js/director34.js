...
const inboxDone=x=>done(x)||['RESOLVED','WAITING'].includes(upper(x?.status||''));
...
function isWork(x){if(x?.scope==='work'||x?.domain==='work'||x?.work===true)return true;return workRx.test(norm(`${titleOf(x)} ${x?.project||''} ${x?.kind||''} ${x?.domain||''} ${x?.from||''} ${x?.sender||''}`))}
