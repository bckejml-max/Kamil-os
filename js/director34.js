...
function occurrence(def,now=new Date()){
 const y=now.getFullYear(),m=now.getMonth(),today=atNoon(y,m,now.getDate()),day=def.rule==='LAST_DAY'?lastDay(y,m):Math.min(def.day,lastDay(y,m));let due=atNoon(y,m,day);
 if(due<today){const nm=m===11?0:m+1,ny=m===11?y+1:y,nd=def.rule==='LAST_DAY'?lastDay(ny,nm):Math.min(def.day,lastDay(ny,nm));due=atNoon(ny,nm,nd)}
 return due;
}
...
export function directorDeadlines34(state={},now=new Date()){
 ...
 for(const d of DIRECTOR_DEADLINES_34){
  let due=occurrence(d,now),key=`${d.id}|${monthKey(due)}`;
  if(completions[key]){const y=due.getFullYear(),m=due.getMonth()+1,ny=m>11?y+1:y,nm=m>11?0:m,nd=d.rule==='LAST_DAY'?lastDay(ny,nm):Math.min(d.day,lastDay(ny,nm));due=atNoon(ny,nm,nd);key=`${d.id}|${monthKey(due)}`}
  const days=diffDays(due,now);rows.push({...d,due:ymd(due),month:monthKey(due),completionKey:key,done:!!completions[key],days,priority:priorityFromDays(days,d.priority)});
 }
...
export function directorBriefing34(state={},now=new Date()){
 ...
 return {top,deadlines:deadlineRows,tasks,waiting:[...delegations,...manual].sort((a,b)=>b.priority-a.priority),calendar,critical:top.filter(x=>x.priority>=90).length,week:all.filter(x=>x.days!==null&&x.days<=7).length,note:'Director Mode kombinuje měsíční ředitelské termíny, pracovní úkoly, čekání na ostatní a pracovní kalendář. Nic pracovního automaticky neodesílá.'};
}
