...
function model(){
 const s=store.get(),d=buildDomainOS328(s),rows=[];
 (d.manager?.overdue||[]).slice(0,4).forEach(x=>rows.push({key:'manager',label:'Práce',title:title(x),reason:'Po termínu · zavřít, delegovat nebo posunout.',score:120+dueScore(x)}));
 (d.tickets?.signals||[]).filter(x=>['BUY','SELL/REPRICE','REVIEW'].includes(String(x.signal||''))).slice(0,4).forEach(x=>rows.push({key:'tickets',label:'Vstupenky',title:title(x),reason:`${x.signal} · ${x.reason||x.sellTiming||'zkontrolovat cenu'}`,score:x.signal==='SELL/REPRICE'?112:x.signal==='BUY'?106:96}));
 (d.property?.urgent||[]).slice(0,3).forEach(x=>rows.push({key:'property',label:'Nemovitosti',title:title(x),reason:'Důležitý otevřený krok kolem nemovitosti.',score:92+dueScore(x)}));
 (d.investments?.rows||[]).filter(x=>x.action&&x.action!=='HOLD').slice(0,3).forEach(x=>rows.push({key:'money',label:'Peníze',title:title(x),reason:`${x.action} · ${x.reason||'zkontrolovat pozici'}`,score:x.action==='REDUCE'?88:82}));
 (s.tasks||[]).filter(x=>!done(x)).filter(x=>!/faktur|zakáz|dodavat|pks|cpi|zbroj|práce|pracovn/i.test(`${x.title||''} ${x.area||''} ${x.category||''}`)).slice(0,4).forEach(x=>rows.push({key:'personal',label:'Osobní',title:title(x),reason:x.due?'Osobní úkol s termínem.':'Otevřený osobní úkol.',score:60+dueScore(x)}));
 if(!rows.length&&num(d.money?.freeCapital)>0)rows.push({key:'money',label:'Peníze',title:`${money(d.money.freeCapital)} volného kapitálu`,reason:'Zkontroluj, jestli kapitál nemá lepší využití.',score:55});
