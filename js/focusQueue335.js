...
function managerDutyRows(){const duties=window.__KAMIL_MANAGER_OS341__?.model?.duties||[];return duties.filter(x=>!x.isDone&&['overdue','today','soon'].includes(x.state)).map(x=>({key:'manager',label:'Práce',title:x.label||title(x),reason:x.state==='overdue'?`${Math.abs(Number(x.diff)||0)} d po termínu · měsíční povinnost.`:x.state==='today'?'Dnes · měsíční povinnost.':`Za ${Number(x.diff)||0} d · měsíční povinnost.`,score:x.state==='overdue'?145:x.state==='today'?136:118-Math.max(0,Number(x.diff)||0),refType:'manager-duty',refId:x.id||null}));}
function model(){
 const s=store.get(),d=buildDomainOS328(s),rows=[],taskIds=new Set((s.tasks||[]).map(x=>x.id));
 managerDutyRows().forEach(x=>rows.push(x));
 ...
 return{version:349,queue,now:queue[0]||null,next:queue.slice(1,4),at:new Date().toISOString()};
}
...
function html(m){
 if(!m.now)return `<section class="os335-queue" data-focus-queue335><div class="os335-head"><div><small>KAMIL OS · 349</small><h2>Focus Queue</h2></div><span>Čisto</span></div><div class="os335-empty">Teď není žádná silná další akce.</div></section>`;
...
function render(){const host=anchor();if(!host)return false;const m=model();host.innerHTML=html(m);window.__KAMIL_FOCUS_QUEUE335__={version:349,model:m,healthy:true,mounted:!!host.querySelector('[data-focus-queue335]'),refresh:renderSafe,open,at:Date.now()};return true}
...
export function installFocusQueue335(){injectCss();document.documentElement.dataset.focusQueue335='1';if(!bound){bound=true;document.addEventListener('click',e=>{const b=e.target.closest?.('[data-focus335-open]');if(!b)return;e.preventDefault();open(b.dataset.focus335Open)});window.addEventListener('kamil:view-change',e=>{if(!e.detail||e.detail==='today')schedule(40)});window.addEventListener('kamil:manager341-updated',()=>schedule(20));store.subscribe?.(()=>schedule())}schedule(30);setTimeout(()=>schedule(20),600);setTimeout(()=>schedule(20),1700)}
