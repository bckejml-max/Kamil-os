...
function render(){const host=anchor();if(!host)return false;const m=model(),markup=html(m),first=!window.__KAMIL_FOCUS_QUEUE335__;let queue=host.querySelector(':scope > [data-focus-queue335]'),changed=false;if(!queue){host.insertAdjacentHTML('afterbegin',markup);queue=host.querySelector(':scope > [data-focus-queue335]');changed=true}else if(queue.outerHTML!==markup){queue.outerHTML=markup;queue=host.querySelector(':scope > [data-focus-queue335]');changed=true}window.__KAMIL_FOCUS_QUEUE335__={version:349,model:m,healthy:true,mounted:!!queue,refresh:renderSafe,open,at:Date.now()};if(first||changed)window.dispatchEvent(new CustomEvent('kamil:focus-queue335-updated',{detail:{version:349,current:m.now?.key||null,changed}}));return true}
