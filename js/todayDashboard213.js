...
function decorate213(){
 const host=document.querySelector('#todayView'),root=host?.querySelector('.ux65-today');if(!root||root.dataset.dashboard213==='1')return false;
 const hero=root.querySelector(':scope > .ux65-hero'),commander=root.querySelector(':scope > .ux66-priority')||root.querySelector(':scope > .ux64-clear'),health=root.querySelector(':scope > .os684-health');
...
function syncLateHealth213(){const root=document.querySelector('#todayView .ux65-today[data-dashboard213="1"]'),shell=root?.querySelector('[data-today-dashboard213]'),health=root?.querySelector(':scope > .os684-health');if(shell&&health)shell.querySelector('.today213-status')?.appendChild(health)}
export function installTodayDashboard213(){ensureStyle213();let timer=0;const run=()=>{clearTimeout(timer);timer=setTimeout(()=>{decorate213();syncLateHealth213()},40)};const host=document.querySelector('#todayView');if(host)new MutationObserver(run).observe(host,{childList:true,subtree:true});window.addEventListener('kamil:view-change',e=>{if(e.detail==='today')run()});run();setTimeout(run,500);}
