...
async function optionalImport(path,fn){const start=performance.now();let ok=false;try{const m=await import(path);if(typeof m?.[fn]==='function')await m[fn]();ok=true;return true}catch(error){noteBootError(path,error);return false}finally{BOOT343.modules.push({path,fn,ms:Math.max(0,Math.round((performance.now()-start)*10)/10),ok});publishBoot343()}}
async function deferredImport(path,fn){const start=performance.now();let ok=false;try{const m=await import(path);if(typeof m?.[fn]==='function')await m[fn]();ok=true;return true}catch(error){console.warn(`[instantShell66:deferred:${path}]`,error);DEFER345.failures.push({scope:path,message:String(error?.message||error)});return false}finally{DEFER345.modules.push({path,fn,ms:Math.max(0,Math.round((performance.now()-start)*10)/10),ok});publishDeferred345()}}
...
function scheduleDeferred345(){
  const run=async()=>{
    ...
    await deferredImport('./capitalPlan469.js','installCapitalPlan469');
    ...
    await deferredImport('./ticketRiskSummary610.js','installTicketRiskSummary610');
