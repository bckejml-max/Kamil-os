...
const mobileMenu=qs('#mobileMenuBtn');if(mobileMenu)mobileMenu.onclick=async()=>{const names={today:'Dnes',work:'Práce',tickets:'Vstupenky',property:'Reality',money:'Peníze',betting:'Sázení',inbox:'Úkoly',family:'Rodina',home:'Domov',more:'Dokumenty'};const choice=await modal('Všechny sekce','',[...Object.entries(names).map(([value,label])=>({label,value,primary:value===current})),{label:'Zavřít',value:null}]);if(choice)navigate(choice)};
