...
const TABLE='kamil_os_history';let lastSyncAt=null,lastError=null,lastUploaded=0,lastCloudCount=0,running=false;
const chunks=(a,n)=>{const out=[];for(let i=0;i<a.length;i+=n)out.push(a.slice(i,i+n));return out};
export function cloudHistoryRows32(records=[],userId='',version=APP_VERSION,updatedAt=new Date().toISOString()){return (Array.isArray(records)?records:[]).filter(x=>x?.key&&x?.bucket).map(x=>({user_id:String(userId),record_key:String(x.key),bucket:String(x.bucket),happened_at:x.at||null,payload:x.payload??{},source_version:String(version),updated_at:updatedAt}))}
export async function syncCloudHistory32(){
 ...
 try{const c=await cloudClient(false);if(!c)return {ok:false,reason:'LOCAL_ONLY'};const sess=(await c.auth.getSession()).data.session;if(!sess)return {ok:false,reason:'NO_SESSION'};const plan=historyPlan31(store.get()),now=new Date().toISOString(),rows=cloudHistoryRows32(plan.records,sess.user.id,APP_VERSION,now);let uploaded=0;for(const batch of chunks(rows,200)){if(!batch.length)continue;const {error}=await c.from(TABLE).upsert(batch,{onConflict:'user_id,record_key'});if(error)throw error;uploaded+=batch.length}const countRes=await c.from(TABLE).select('record_key',{count:'exact',head:true});if(countRes.error)throw countRes.error;lastSyncAt=new Date().toISOString();lastUploaded=uploaded;lastCloudCount=Number(countRes.count||0);lastError=null;return {ok:true,uploaded,cloudCount:lastCloudCount,lastSyncAt,counts:plan.counts}}
...
export const cloudHistory32Info={table:TABLE,buckets:['decision','networth','ticket','trade','import'],mode:'DUAL_WRITE',deleteEnabled:false,sourceOfTruth:'kamil_os_state + IndexedDB until cutover'};
