...
export async function syncCloudHistory32(){
 ...
 try{const c=await cloudClient(false);if(!c)return {ok:false,reason:'LOCAL_ONLY'};const sess=(await c.auth.getSession()).data.session;if(!sess)return {ok:false,reason:'NO_SESSION'};const plan=historyPlan31(store.get()),rows=plan.records.map(x=>({user_id:sess.user.id,record_key:x.key,bucket:x.bucket,happened_at:x.at||null,payload:x.payload??{},source_version:APP_VERSION,updated_at:new Date().toISOString()}));let uploaded=0;for(const batch of chunks(rows,200)){if(!batch.length)continue;const {error}=await c.from(TABLE).upsert(batch,{onConflict:'user_id,record_key'});if(error)throw error;uploaded+=batch.length}const countRes=await c.from(TABLE).select('record_key',{count:'exact',head:true});if(countRes.error)throw countRes.error;lastSyncAt=new Date().toISOString();lastUploaded=uploaded;lastCloudCount=Number(countRes.count||0);lastError=null;return {ok:true,uploaded,cloudCount:lastCloudCount,lastSyncAt,counts:plan.counts}}
