...
export function ticketRepricingGuard194(row={},learning,source={},now=Date.now()){
 ...
 else if(ask<hardFloor){action='RAISE TO';recommendedAsk=hardFloor;reason='Aktuální ask je pod normálním +50 % ROI floorem; cena se musí vrátit nad guard.';}
 else {
  const delta=(target-ask)/ask;
  if(delta>=.06){action='RAISE TO';reason='Trh dovoluje vyšší cenu; navyšuji jen k dynamickému cíli.';}
  else if(delta<=-.06){action='DROP TO';reason=target===hardFloor?'Market je níž, ale guard blokuje pokles pod +50 % ROI floor.':'Ask je nad časově upraveným market cílem; snižuji, ale nikdy pod +50 % ROI floor.';}
  else recommendedAsk=ask;
 }
 if(recommendedAsk<hardFloor)recommendedAsk=hardFloor;
 return{id:row?.id||'',name:name(row),section:String(row?.section||'—'),qty:q,askEach:ask,marketEach:market,days,action,recommendedAsk:round10(recommendedAsk),neverBelow:hardFloor,emergencyFloor,hardFloor,marketFactor:factor,reason,floors:{breakEven,roi20,roi50}};
}
