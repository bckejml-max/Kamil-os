...
export function mountSourceBadge(domain,host){if(typeof host==='string')host=document.querySelector(host);if(!host)return false;const x=sourceState(domain),id=`source-${String(domain).toLowerCase()}`,old=host.querySelector(`[data-source-badge="${id}"]`),html=`<div class="data-source-strip" data-source-badge="${id}"><span class="eyebrow">DATA</span><span>${h(x.source)}</span><b>${h(x.confidence)}</b><span>${h(x.freshness.label)}${x.freshness.ageDays!==null?` · ${x.freshness.ageDays} d`:''}</span><span class="muted">${h(x.detail)}</span></div>`;if(old)old.outerHTML=html;else{const head=host.querySelector('.view-head');head?.insertAdjacentHTML('afterend',html)||host.insertAdjacentHTML('afterbegin',html)}return true}
