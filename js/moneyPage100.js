...
function enhance(){if(running){rerun=true;return}running=true;queueMicrotask(async()=>{try{installXtbLayers();appendWealthHistory610();appendPropertyFinance610();appendPropertyHub620();appendXtbIntelligence100();appendXtbDetails100();await appendCapitalBrain100();applyMoneyDataQa144();enhanceMoneyVisual138();await enhanceUnifiedCapital160();await enhanceRecommendationPerformance162();await enhanceMoney181();await enhanceMoney181Final()}catch(e){console.warn('[money620]',e)}finally{running=false;if(rerun){rerun=false;enhance()}}})}
function scheduleEnhance(){for(const t of timers)clearTimeout(t);timers=[setTimeout(enhance,40),setTimeout(enhance,500)];}
export function renderMoneyPage100(){ensureStyle();renderPersonalMoney640();installXtbLayers();appendWealthHistory610();appendPropertyFinance610();appendPropertyHub620();scheduleEnhance()}
