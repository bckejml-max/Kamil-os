...
const closeOnVersionChange=db=>{db.onversionchange=()=>{try{db.close()}catch{}};return db};
...
export async function smartSyncContext31(){
 if(!indexedDbSupported31())return {supported:false,baseline:false,records:[],seq:0,pending:0,uploaded:0,lastDiffAt:null,lastUploadAt:null,lastCloudCount:0};
...
export async function resetSmartSyncBaseline31(records=[],{seq=0,discardOps=false}={}){
 const db=await openDataDb31();try{const tx=db.transaction([SYNC_SHADOW,SYNC_OPS,META],'readwrite'),done=txDone(tx),shadow=tx.objectStore(SYNC_SHADOW),ops=tx.objectStore(SYNC_OPS),meta=tx.objectStore(META),now=new Date().toISOString(),mr=meta.get('smart-sync');shadow.clear();for(const x of Array.isArray(records)?records:[])shadow.put(x);if(discardOps)ops.clear();mr.onsuccess=()=>meta.put({...mr.result,key:'smart-sync',baseline:true,seq:Math.max(0,Number(seq)||0),lastDiffAt:now,...(discardOps?{lastUploadAt:null,lastCloudCount:0}:{})});await done;return {ok:true,baseline:true,total:Array.isArray(records)?records.length:0}}finally{db.close()}
}
export async function commitSmartSyncDiff31(records=[],ops=[],{nextSeq=0}={}){
 const db=await openDataDb31();try{const tx=db.transaction([SYNC_SHADOW,SYNC_OPS,META],'readwrite'),done=txDone(tx),shadow=tx.objectStore(SYNC_SHADOW),opStore=tx.objectStore(SYNC_OPS),meta=tx.objectStore(META),now=new Date().toISOString(),mr=meta.get('smart-sync');shadow.clear();for(const x of Array.isArray(records)?records:[])shadow.put(x);for(const x of Array.isArray(ops)?ops:[])opStore.put(x);mr.onsuccess=()=>meta.put({...mr.result,key:'smart-sync',baseline:true,seq:Math.max(0,Number(nextSeq)||0),lastDiffAt:now});await done;return {ok:true,written:Array.isArray(ops)?ops.length:0}}finally{db.close()}
}
...
export async function markSmartSyncUploaded31(ids=[],{serverAt=new Date().toISOString(),cloudCount=0}={}){
 const wanted=new Set(Array.isArray(ids)?ids:[]),db=await openDataDb31();try{const tx=db.transaction([SYNC_OPS,META],'readwrite'),done=txDone(tx),ops=tx.objectStore(SYNC_OPS),meta=tx.objectStore(META),mr=meta.get('smart-sync');for(const id of wanted){const r=ops.get(id);r.onsuccess=()=>{if(r.result)ops.put({...r.result,status:'UPLOADED',uploadedAt:serverAt})}}mr.onsuccess=()=>meta.put({...mr.result,key:'smart-sync',lastUploadAt:serverAt,lastCloudCount:Number(cloudCount||0)});await done;return {ok:true,uploaded:wanted.size,lastUploadAt:serverAt,lastCloudCount:Number(cloudCount||0)}}finally{db.close()}
}
...
export async function remoteInboxMeta31(){
