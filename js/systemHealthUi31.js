...
function render(){
 const view=qs('#moreView');if(!view)return;const isSystem=[...view.querySelectorAll('h1')].some(x=>String(x.textContent||'').includes('Kamil OS'));if(!isSystem){qs(`#${id}`,view)?.remove();return}
 qs(`#${id}`,view)?.remove();const s=store.get(),r=systemHealth31(s,store.meta(),{stateBytes:storedBytes(LOCAL_KEY),queueBytes:storedBytes(QUEUE_KEY),pendingSync:!!store.readQueue(),online:navigator.onLine,serviceWorkerSupported:'serviceWorker'in navigator,serviceWorkerControlled:!!navigator.serviceWorker?.controller,schemaVersion:SCHEMA_VERSION,releaseConsistent:true});
 const el=document.createElement('div');el.id=id;el.className='card';el.innerHTML=`<div class="card-head"><div><div class="eyebrow">CORE V2 / SYSTEM HEALTH 31</div><h2>Health score ${r.score}/100</h2><p class="muted">Diagnostika běhu Kamil OS ${h(APP_VERSION)}. Nic sama neopravuje ani neposílá.</p></div><span class="status ${tone(r.status)}">${label(r.status)}</span></div>${r.checks.map(x=>`<div class="row"><span>${h(x.label)}<small class="muted" style="display:block">${h(x.detail)}</small></span><b class="${tone(x.level)}">${label(x.level)}</b></div>`).join('')}<div class="decision-note">State ${Math.max(1,Math.round(r.stateBytes/1024)).toLocaleString('cs-CZ')} kB · pending sync ${Math.round(r.queueBytes/1024).toLocaleString('cs-CZ')} kB. Nad ~3 MB už Kamil OS varuje před limity browserového úložiště.</div>`;
 const target=view.querySelector('.view-head');if(target)target.insertAdjacentElement('afterend',el);else view.prepend(el);
}
function start(){const view=qs('#moreView');if(!view)return;new MutationObserver(()=>queueMicrotask(render)).observe(view,{childList:true,subtree:false});window.addEventListener('kamil:navigate',e=>{if(e.detail==='more')queueMicrotask(render)});window.addEventListener('online',render);window.addEventListener('offline',render);render()}
