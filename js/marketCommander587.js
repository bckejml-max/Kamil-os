...
export function commanderBlockerResolver591(s=store.get()){
 const d=commanderDecision590(s),r=recheckTriggers562(s),rows=[...r.verifyNow,...r.waiting].filter(personal),row=rows[0]||null,primary=row?.primary||null;
 const result=d.mode==='ACT'?{blocked:false,mode:'READY',when:'TEĎ',text:'Rozhodnutí prošlo všemi gate; další krok je ruční provedení podle Commanderu.',name:d.what,kind:'READY'}:primary?{blocked:true,mode:row.mode,when:primary.when,text:primary.text,name:row.name||row.ticker||'Market položka',kind:primary.kind}:{blocked:true,mode:d.mode,when:d.mode==='VERIFY'?'TEĎ':'PŘI NOVÝCH DATECH',text:d.whatNext||'Doplň chybějící market data a spusť Commander znovu.',name:d.what,kind:'FALLBACK'};
 window.__KAMIL_COMMANDER_BLOCKER_591_LAST__={at:Date.now(),blocked:result.blocked,kind:result.kind,when:result.when};return result;
}
