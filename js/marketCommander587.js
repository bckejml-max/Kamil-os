...
export function capitalReusePlanner568(s=store.get()){
 ...
 const opportunities={xtb:xtbOpportunityScores(s).filter(x=>x.score>=65).slice(0,3),tickets:ticketBuyOpportunities(s).filter(x=>x.score>=65).slice(0,3)};
...
export async function openMarketCommander587(){
 const x=marketCommander587(),heat=portfolioHeatmap572(),liq=ticketLiquidityScore579(),bands=ticketPriceBands580(),reuse=capitalReusePlanner568(),body=`<div class="metric-strip"><div class="metric"><span>Režim</span><b class="${x.next.mode==='ACT'?'good':x.next.mode==='VERIFY'?'warn':''}">${h(x.next.mode)}</b></div><div class="metric"><span>Risk kroky</span><b>${x.risk.total}</b></div><div class="metric"><span>Confidence</span><b>${x.next.confidence??'—'}${x.next.confidence!==null?' %':''}</b></div><div class="metric"><span>Top ranking</span><b>${h(x.ranking[0]?.name||'—')}</b></div></div><div class="card"><div class="eyebrow">MARKET COMMANDER 58.7</div><h2>${h(x.next.instruction)}</h2><p>${h(x.next.why)}</p>${x.next.qty?`<p><b>Množství:</b> ${h(x.next.qty)}</p>`:''}${x.next.price?`<p><b>Cena:</b> ${h(fmt(x.next.price))}</p>`:''}</div><div class="card"><div class="eyebrow">56.8–57.7 · XTB + KAPITÁL</div>${simpleRows(heat,x=>`${x.ticker} · heat ${x.heat}`)}</div><div class="card"><div class="eyebrow">57.8–58.5 · TICKETY</div>${simpleRows(liq,x=>`${x.name} · likvidita ${x.score}`)}${simpleRows(bands,x=>`${x.name} · ${x.status}`)}</div><div class="card"><div class="eyebrow">58.6 · CROSS-MARKET</div>${simpleRows(x.ranking,x=>`${x.domain} · ${x.name} · ${x.score}`)}</div><div class="card"><div class="eyebrow">56.8 · UVOLNĚNÝ KAPITÁL</div>${simpleRows(reuse.currencies,x=>`${x.currency} · net ${fmt(x.net,x.currency)}`)}</div><div class="decision-note">58.7 skládá všech 20 nových vrstev pouze z uložených dat. Market depth, sellout a historie jsou NEZNÁMÉ, pokud je nemáš uložené. Nic automaticky nenakupuje, neprodává, nepřevádí měny ani nepřecenňuje.</div>`;return modal('XTB + vstupenky / Market Commander 58.7',body,[{label:'Zavřít',value:null,primary:true}]);
}
