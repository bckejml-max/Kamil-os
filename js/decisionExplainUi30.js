...
async function showWhy(index){
 const decision=current()[Number(index)];if(!decision)return toast('Rozhodnutí už není v aktuálním Top 3.');
 const e=decisionExplain30(decision),facts=e.facts.map(x=>`<div class="decision-note"><div>${h(x)}</div></div>`).join('')||'<div class="empty">Původní scoring neposkytuje další strukturovaný fakt. 30.3 nic nedoplňuje odhadem.</div>';
 const body=`<div class="metric-strip"><div class="metric"><span>Priorita</span><b>${e.score}/100</b></div><div class="metric"><span>Engine</span><b>${h(e.engine)}</b></div></div><div class="decision-note" style="margin-top:12px"><b>Pravidlo skóre</b><div>${h(e.rule)}</div></div><h3>Skutečná fakta</h3>${facts}${metaRow('Zdroj',e.source)}${metaRow('Confidence',e.confidence)}<p class="muted">${h(decisionExplain30Note)}</p>`;
...
function decorate(){
 ...
 const rows=[...card.children].filter(x=>x.classList?.contains('autopilot29-row'));
 rows.forEach((row,index)=>{
  if(row.querySelector(`[${BUTTON_ATTR}]`))return;
  const b=document.createElement('button');b.type='button';b.className='btn';b.textContent='Proč teď?';b.setAttribute(BUTTON_ATTR,String(index));b.setAttribute('aria-label',`Vysvětlit prioritu rozhodnutí ${index+1}`);b.addEventListener('click',()=>showWhy(index));
