...
export function renderTasksOverview(){
 ...
 '<section class="pr1320-now"><div><div class="pr1300-kicker">Teď</div><h2>'+esc(top?.title||'Nic akutního')+'</h2><p>'+esc(top?.detail||quietDetail)+'</p></div><div class="pr1320-now-actions"><button class="pr1300-btn primary" type="button" data-task-add>＋ Úkol</button><button class="pr1300-btn" type="button" data-task-wait>Čekám na někoho</button></div></section>' +
 ...
 '<section class="pr1300-panel"><div class="pr1300-panel-head"><h2>Fronta</h2><button class="pr1300-btn" type="button" data-task-advanced>Gmail + detail</button></div>'+rows(m.rows)+'</section></div>';
 if(!host.dataset.tasksOverviewBound){host.dataset.tasksOverviewBound='1';ownEvent1100(OWNER,host,'click',async e=>{if(e.target.closest('[data-task-add]')){window.dispatchEvent(new CustomEvent('kamil:capture',{detail:'task'}));return}if(e.target.closest('[data-task-wait]')){window.dispatchEvent(new CustomEvent('kamil:capture',{detail:'waiting'}));return}const rowBtn=e.target.closest('[data-task-open]');if(rowBtn){const row=lastModel?.rows?.find(x=>x.id===rowBtn.dataset.taskOpen);if(row?.insurance){window.dispatchEvent(new CustomEvent('kamil:navigate',{detail:'more'}));schedule1100(OWNER,'insurance-open',async()=>{const m=await import('./insuranceUi25.js');m.renderInsurance25?.()},140,{pauseWhenHidden:true});return}if(row?.action){await openPersonalAction641(row.action);renderTasksOverview();return}window.dispatchEvent(new CustomEvent('kamil:navigate',{detail:row?.route||rowBtn.dataset.taskRoute||'today'}));return}if(e.target.closest('[data-task-advanced]')){host.dataset.productAdvanced='1';await loadProductAdvancedStyles(["./core70.css","./personal64.css"]);const m=await import('./inboxAdvanced141.js');await m.renderInboxPage141?.()}})}
