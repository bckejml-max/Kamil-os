...
function rows(items){if(!items.length)return '<div class="pr1300-empty">Nic otevřeného.</div>';return items.slice(0,8).map(x=>'<button type="button" class="pr1300-row pr1300-clickrow" data-task-route="'+esc(x.route||'today')+'"><div class="pr1300-row-main"><b>'+esc(x.title)+'</b><small>'+esc(x.sourceLabel||x.bucket||'Úkol')+(x.detail?' · '+esc(x.detail).slice(0,90):'')+'</small></div><div class="pr1300-row-side '+(x.days!==null&&x.days<0?'bad':x.days===0?'warn':'')+'">'+esc(when(x))+'</div></button>').join('')}
