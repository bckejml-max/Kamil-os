...
function data(){
 ...
 const urgent=timeline.filter(x=>x.days<=30).sort((a,b)=>a.days-b.days),primary=urgent[0]||null;
 return {s,records,timeline,maintenance,urgent,primary,overdue:timeline.filter(x=>x.days<0).length,next90:timeline.filter(x=>x.days>=0&&x.days<=90).length};
}
const timelineRows=rows=>rows.length?rows.slice(0,10).map(x=>`<div class="pr1300-row"><div class="pr1300-row-main"><b>${h(x.title)}</b><small>${h(x.kind==='maintenance'?'Údržba / servis':'Smlouva / termín')}</small></div><div class="pr1300-row-side ${x.days<0?'bad':x.days<=14?'warn':''}">${x.days<0?`${Math.abs(x.days)} d po termínu`:x.days===0?'dnes':x.days===1?'zítra':`za ${x.days} d`}</div></div>`).join(''):'<div class="os1500-empty">V příštích 12 měsících není uložený známý termín kolem domova.</div>';
