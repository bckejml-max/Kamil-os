...
function data(){
 ensurePersonalVault640();const s=store.get(),vault=personalVault640(s),insurance=insuranceCenter(s),vaultRecords=vault.records.filter(x=>x.section==='home'&&x.status?.code!=='ARCHIVED'),insuranceRecords=insurance.policies.filter(x=>x.kind==='PROPERTY').map(x=>({id:x.id,source:'insurance',recordType:'insurance',title:x.title,provider:x.provider,premium:x.premium,cadence:x.cadence,monthlyAmount:x.monthlyPremium,annualAmount:x.annualPremium,nextAction:x.issues?.[0]||'Otevřít v Pojištění.',statusLabel:x.lifecycleLabel})),records=[...vaultRecords,...insuranceRecords],timeline=personalHomeTimeline650(s);
...
const homeAmount=x=>x.source==='insurance'&&x.premium!==null&&x.premium!==undefined?(x.cadence==='MONTHLY'?money(x.premium)+'/měs. · ':x.cadence==='YEARLY'?money(x.premium)+'/rok · ':money(x.premium)+' · '):(x.monthlyAmount?money(x.monthlyAmount)+'/měs. · ':x.annualAmount?money(x.annualAmount)+'/rok · ':'');
const recordRows=rows=>rows.length?rows.map((x,i)=>`<button type="button" class="os1500-record" data-home1500-record="${i}"><span>${h(x.recordType==='insurance'?'Pojištění':x.recordType==='utility'?'Energie':x.recordType==='property'?'Nemovitost':'Domov')}</span><b>${h(x.title)}</b><small>${homeAmount(x)}${h(x.statusLabel?x.statusLabel+' · ':'')}${h(x.nextAction||'Otevřít a zkontrolovat')}</small></button>`).join(''):'<div class="os1500-empty">Zatím nejsou uložené smlouvy ani údaje k domovu.</div>';
