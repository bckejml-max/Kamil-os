...
const VERSION='1220.0.0',OWNER='core.performance1220';
...
function measure(view,start){requestAnimationFrame(()=>requestAnimationFrame(()=>{const ms=Math.round((performance.now()-start)*10)/10),budget=BUDGETS[view]||BUDGETS.unknown,row={view,ms,budget,ok:ms<=budget,at:Date.now()};state.views[view]=row;state.last=row;if(!row.ok){state.violations.push(row);trim();globalThis.dispatchEvent?.(new CustomEvent('kamil:performance-budget1220',{detail:row}))}}))}
function onView(e){const view=String(e?.detail||'unknown');measure(view,performance.now())}
export function performanceBudget1220(){const health=runtimeHealth1120(),rows=Object.values(state.views),violations=state.violations.slice(-20);return{version:VERSION,budgets:{...BUDGETS},views:{...state.views},last:state.last,violations,violationCount:state.violations.length,slowViews5m:health.slowViews5m,tone:violations.length?'warn':'good'}}
