...
installStateContracts();installCommandRouter();
...
shellContract();cleanup('consolidation-runtime');listen('consolidation-runtime',window,'kamil:boot-budget343',onCoreReady,{once:true});if(window.__KAMIL_BOOT_BUDGET343__?.complete)queueMicrotask(onCoreReady);else setTimeout(()=>{if(!state.coreReady)onCoreReady()},1800);
