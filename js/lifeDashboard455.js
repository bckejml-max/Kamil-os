...
import {openPersonalLifeHome622} from './personalLifeHome622.js';
...
export async function openLifeDashboard455(){
 ...
 const body=`<div class="metric-strip"><div class="metric"><span>Readiness</span><b>${x.command.readiness}/100</b></div><div class="metric"><span>Life Health</span><b>${x.plus.health.score}/100</b></div><div class="metric"><span>Safe purchase</span><b>${money(x.assistant.purchase.limit)}</b></div><div class="metric"><span>Investice / měsíc</span><b>${money(x.assistant.contribution.suggested)}</b></div></div><div class="card"><div class="eyebrow">PERSONAL LIFE HOME 62.2</div><h2>Osobní život má vlastní hlavní obrazovku</h2><p class="muted">Jedna nejlepší osobní věc teď + rychlý přehled nejdůležitějších oblastí + přístup ke všem 20 modulům.</p></div><div class="card"><div class="eyebrow">UDĚLEJ TEĎ · TOP 3 DNES</div>${top}</div><div class="card"><div class="eyebrow">CO HLÍDAT</div>${alert}</div><div class="card"><div class="eyebrow">CO SE BLÍŽÍ / 3 MĚSÍCE</div>${next}</div><div class="card"><div class="eyebrow">RYCHLÝ STAV</div><div class="row"><span>Pokrytí dat</span><b>${x.practical.missing.coverage}%</b></div><div class="row"><span>Cashflow za 90 dní</span><b>${money(x.cashflow.d90.end)}</b></div><div class="row"><span>Záruky do 120 dní</span><b>${x.assistant.warranty.length}</b></div></div><div class="decision-note">Životní dashboard 62.2 zůstává click-only/read-only. Zápisy jsou dál pouze v původních modulech po explicitním potvrzení.</div>`;
 const choice=await modal('Kamil OS / Životní dashboard 62.2',body,[{label:'Personal Life Home',value:'life622',primary:true},{label:'Life Commander 62.1',value:'life621'},{label:'Zeptat se asistenta',value:'assistant'},{label:'Quick Capture',value:'capture'},{label:'Assistant Suite 30',value:'suite'},{label:'Udělej teď',value:'command'},{label:'Praktické centrum',value:'practical'},{label:'Life+ 20',value:'plus'},{label:'Finance',value:'finance'},{label:'Plány',value:'plan'},{label:'Rodina',value:'family'},{label:'Vstupenky',value:'tickets'},{label:'Inbox',value:'inbox'},{label:'Další',value:'more'},{label:'Zavřít',value:null}]);
 if(choice==='life622')return openPersonalLifeHome622();
