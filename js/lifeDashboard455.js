...
export async function openLifeDashboard455(){
 ...
 const body=`<div class="metric-strip"><div class="metric"><span>Readiness</span><b>${x.command.readiness}/100</b></div><div class="metric"><span>Life Health</span><b>${x.plus.health.score}/100</b></div><div class="metric"><span>Safe purchase</span><b>${money(x.assistant.purchase.limit)}</b></div><div class="metric"><span>Investice / měsíc</span><b>${money(x.assistant.contribution.suggested)}</b></div></div><div class="card"><div class="eyebrow">UDĚLEJ TEĎ · TOP 3 DNES</div>${top}</div><div class="card"><div class="eyebrow">CO HLÍDAT</div>${alert}</div><div class="card"><div class="eyebrow">CO SE BLÍŽÍ / 3 MĚSÍCE</div>${next}</div><div class="card"><div class="eyebrow">RYCHLÝ STAV</div><div class="row"><span>Pokrytí dat</span><b>${x.practical.missing.coverage}%</b></div><div class="row"><span>Cashflow za 90 dní</span><b>${money(x.cashflow.d90.end)}</b></div><div class="row"><span>Ticket k prověření</span><b>${x.assistant.rotation[0]?h(x.assistant.rotation[0].name):'—'}</b></div><div class="row"><span>Záruky do 120 dní</span><b>${x.assistant.warranty.length}</b></div></div><div class="decision-note">Unified Life Dashboard 53.0: analytické motory jsou click-only/read-only. Quick Capture, Hotovo a Odložit zapisují pouze po explicitním potvrzení a přes Undo + audit.</div>`;
 const choice=await modal('Kamil OS / Životní dashboard 53.0',body,[{label:'Zeptat se asistenta',value:'assistant'},{label:'Quick Capture',value:'capture'},{label:'Assistant Suite 30',value:'suite'},{label:'Udělej teď',value:'command'},{label:'Praktické centrum',value:'practical'},{label:'Life+ 20',value:'plus'},{label:'Finance',value:'finance'},{label:'Plány',value:'plan'},{label:'Rodina',value:'family'},{label:'Vstupenky',value:'tickets'},{label:'Inbox',value:'inbox'},{label:'Další',value:'more'},{label:'Zavřít',value:null,primary:true}]);
