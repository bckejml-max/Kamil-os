...
function syncChrome142(view){
 if(typeof document==='undefined')return;const def=viewDefinition41(view),p=document.querySelector('#pageTitle');if(p)p.textContent=def.title||'KAMIL OS';
 document.querySelectorAll('[data-view]').forEach(x=>{const on=x.dataset.view===view;x.toggleAttribute('aria-current',on);if(on)x.setAttribute('aria-current','page')});
 const add=document.querySelector('#quickAddBtn');if(add){const hidden=!def.quickLabel;add.classList.toggle('hidden',hidden);if(!hidden){const b=add.querySelector('b');if(b)b.textContent=def.quickLabel;add.title=`Rychle přidat ${def.quickLabel.toLowerCase()} · Ctrl N`}}
}
ensurePolish142();syncChrome142('today');window.addEventListener?.('kamil:view-change',e=>syncChrome142(e.detail));
...
function warmView(name='today'){const key=validViews41.has(name)?name:'today';if(warmViews.has(key))return warmViews.get(key);const def=viewDefinition41(key),p=Promise.resolve().then(()=>hydrateColdView42(key)).then(()=>load(def.renderer[0])).then(m=>{const renderer=m?.[def.renderer[1]];if(typeof renderer!=='function')throw new Error(`Chybí renderer ${def.renderer[1]} pro ${key}`);return renderer}).catch(err=>{warmViews.delete(key);throw err});warmViews.set(key,p);return p}
...
export async function openCapture41(type='task'){
 ...
 if(type==='document-source'){const btn=document.querySelector('#documentInbox650');if(btn){btn.click();return true}return null}
 const m=await load('./personalCapture643.js');if(type==='family-task')return m.openPersonalCapture643('task',{area:'Rodina',category:'Rodina'});if(type==='home-task')return m.openPersonalCapture643('task',{area:'Domov',category:'Domov'});if(type==='ticket-task')return m.openPersonalCapture643('task',{area:'Vstupenky',category:'Vstupenky'});return m.openPersonalCapture643(['task','waiting','admin','insurance','contract'].includes(type)?type:'task')
}
