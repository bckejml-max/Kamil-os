...
export function renderExtras41(view){const run=()=>{const jobs=[];if(view!=='today')jobs.push(load('./autopilotUi28.js').then(m=>m.renderAutopilot?.(view)));jobs.push(load('./personalPlusUi29.js').then(m=>m.renderPersonalPlus?.(view)));return Promise.allSettled(jobs)};if(view==='today'){idle(run,2200);return Promise.resolve([])}return run()}
let riskTimer=null;
...
export function refreshRiskBadge41(state,delay=1200){clearTimeout(riskTimer);riskTimer=setTimeout(()=>doRiskBadge(state),delay);return Promise.resolve(null)}
export async function runPreflight41(){const m=await load('./preflight.js');return m.runPreflight?.()}
let notifyTimer=null;
export function scheduleNotifications41(delay=420){clearTimeout(notifyTimer);notifyTimer=setTimeout(()=>{Promise.allSettled([load('./autopilotUi28.js').then(m=>m.runAutopilotNotifications?.()),load('./personalPlusUi29.js').then(m=>m.runReminderNotifications?.())])},delay)}
export function warmRuntime41(){idle(()=>Promise.allSettled([load('./personalRisk25.js'),load('./autopilotUi28.js'),load('./personalPlusUi29.js')]),4800)}
