...
function czItems(n){if(n===1)return'1 věc';if(n>=2&&n<=4)return`${n} věci`;return`${n} věcí`}
function build(root){if(root.querySelector('.os139-exec'))return;const head=root.querySelector('.d110-head');if(!head)return;
 const tickets=root.querySelector('.d110-grid-mid .d110-card:first-child'),capital=root.querySelector('.d110-grid-mid .d110-card:nth-child(2)'),today=root.querySelector('.d110-bottom .d110-card:nth-child(2)');
 ...
 exec.innerHTML=`<div class="os139-hero"><div><div class="eyebrow">DNES · EXECUTIVE BRIEFING</div><h1>${actionCount?`${czItems(actionCount)} k pozornosti`:'Žádná známá položka v tomto přehledu'}</h1><p>${firstAction?`${firstAction}${firstActionCta?` · ${firstActionCta}`:''}`:'Níže jsou zobrazené moduly a data, která má OS aktuálně načtená.'}</p></div><div class="os139-live"><i class="${live?'':'unknown'}"></i><span>${live||'stav trhu není potvrzený'}</span></div></div><div class="os139-focusgrid">${focusCard('xtb','XTB',positionCount?`${positionCount} pozic`:'—',live||'stav není potvrzený','blue')}${focusCard('tickets','Vstupenky',ticketCount?`${ticketCount} zobrazeno`:'—','zobrazené ticket řádky','purple')}${focusCard('capital','Kapitál',capitalTop||'Čeká na data','další alokace','green')}${focusCard('today','Dnes',actionCount?`${czItems(actionCount)}`:'Bez známé položky',firstAction||'v tomto přehledu není další akce',actionCount?'warn':'green')}</div><div class="os139-quick"><button data-os139-jump="xtb">Investice</button><button data-os139-jump="tickets">Vstupenky</button><button data-os139-jump="capital">Peníze</button><button data-os139-jump="today">Úkoly dnes</button></div>`;
 head.after(exec);
 ...
 window.__KAMIL_OS139__={at:Date.now(),positions:positionCount,ticketsShown:ticketCount,actionsShown:actionCount};
}
