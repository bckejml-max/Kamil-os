...
export function changePulse35(state={},sinceRaw=null,now=new Date()){
 ...
 for(const m of state.inbox||[]){
  ...
  push({id:`inbox:${m.id||at}`,kind:'E-mail',title:m.subject||m.title||'Nový e-mail',detail:`${m.from||m.sender||'Inbox'}${m.unread===false?'':' · nepřečtené'}`,at,priority:important?90:72,target:'director'});
 }

 const waiting=waitingFor35(state,now);
 for(const w of waiting.rows){
  if(!w.replyDetected||!after(w.replyAt,since))continue;
  push({id:`reply:${w.id}:${w.replyAt}`,kind:'Waiting For',title:`Přišla odpověď · ${w.title}`,detail:`${w.replyFrom?`Od ${w.replyFrom} · `:''}${w.linkConfidence?`shoda ${w.linkConfidence} % · `:''}čekání můžeš zkontrolovat a uzavřít.`,at:w.replyAt,priority:94,target:'waiting'});
 }
 ...
 return {since:since.toISOString(),until:now.toISOString(),items:dedup.slice(0,20),top:dedup.slice(0,5),total:dedup.length,high:high.length,emails:emails.length,finance:finance.length,tickets:tickets.length,note:'Change Pulse porovnává tuto relaci s posledním otevřením na tomto zařízení. Zobrazuje změny uložené v OS, nové interní e-maily, rozpoznané odpovědi, nové XTB importy a snapshoty vstupenek.'};
}
