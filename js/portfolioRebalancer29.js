...
function portfolioPositions(s={}){
 const accounts=s.xtbHub?.accounts||s.xtbHub?.report?.accounts||{},out=[];
 for(const [accountId,a] of Object.entries(accounts))for(const p of a?.positions||[])if(active(p))out.push({...p,ticker:p.ticker||p.symbol||'',accountId,accountCurrency:ccy(a.currency||p.currency||'CZK')});
 return out;
}
...
function projectToSimplex(desired,total){
 ...
 if(Math.abs(diff)>1e-7){const key=keys.sort((a,b)=>out[b]-out[a])[0];out[key]=Math.max(0,out[key]+diff)}
 return out;
}
...
export function portfolioRebalancePlan(s={},opts={}){
 ...
 const raw=portfolioPositions(s);if(!raw.length)return emptyResult('NO_POSITIONS','V XTB nejsou aktivní pozice, podle kterých lze rebalancovat.',currency,targets,targetValidation,{contribution});
