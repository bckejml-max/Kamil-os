...
export async function openPersonalAction641(action){
 ...
 const buttons=[{label:'Hotovo',value:'done',primary:true},{label:'Za 3 dny',value:'3d'},{label:'Za týden',value:'7d'}];if(action.kind!=='waiting')buttons.push({label:'Čekám na odpověď',value:'waiting'});buttons.push({label:'Zavřít',value:null});
 ...
 if(choice==='done'){if(completePersonalAction641(action)){toast('Hotovo.');return 'done'}toast('Položku se nepodařilo najít.')}if(choice==='3d'||choice==='7d'){const days=choice==='3d'?3:7;if(postponePersonalAction642(action,days)){toast(`Posunuto o ${days} dní.`);return 'postponed'}}if(choice==='waiting'){if(markPersonalWaiting642(action,3)){toast('Přidáno do čekání. Follow-up za 3 dny.');return 'waiting'}}return choice;
}
