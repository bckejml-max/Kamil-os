...
export async function appendTicketPresaleExecution200(host=document.querySelector('#ticketIntelView')){
 ...
 const plan=buildTicketPresaleExecution200(radar,cloud.inventory||[],ticketBook,{eventCapPct:20,hardQtyCap:8});
 ...
 sec.innerHTML=`<div class="eyebrow">OS 200 · PRESALE EXECUTION PLAN</div><h2>Kolik maximálně koupit</h2><p class="muted">BUY TARGET převádí na konkrétní limit. Výchozí risk cap je 20 % známého ticket kapitálu na jeden event a max. 8 ks. Bez známého kapitálu OS nákup nevymyslí.</p><div class="metric-strip"><div class="metric"><span>Ticket kapitál</span><b>${plan.capital.total?money(plan.capital.total):'NEZNÁMÝ'}</b></div><div class="metric"><span>Zdroj kapitálu</span><b>${h(plan.capital.source)}</b></div><div class="metric"><span>EXECUTE</span><b>${plan.summary.execute}</b></div><div class="metric"><span>SET CAPITAL</span><b>${plan.summary.setCapital}</b></div></div>${plan.rows.filter(v=>['BUY TARGET'].includes(String(v.action).toUpperCase())||['EXECUTE','SET CAPITAL','TOO LARGE','DATA NEEDED'].includes(v.execution.verdict)).slice(0,8).map(v=>`<div class="card" style="margin-top:12px"><div style="display:flex;justify-content:space-between;gap:14px"><div><b>${h(v.name||'Presale event')}</b><div class="muted">${h(v.stage)} · Opportunity ${v.opportunity?.score||0}/100</div></div><b>${h(v.execution.verdict)}</b></div><div class="metric-strip" style="margin-top:10px"><div class="metric"><span>Max cena / ks</span><b>${v.execution.buyPrice?money(v.execution.buyPrice):'—'}</b></div><div class="metric"><span>Max kusů</span><b>${v.execution.maxQty||'—'}</b></div><div class="metric"><span>Budget eventu</span><b>${v.execution.eventBudget?money(v.execution.eventBudget):'—'}</b></div><div class="metric"><span>Nasadit max.</span><b>${v.execution.deployCapital?money(v.execution.deployCapital):'—'}</b></div></div><div class="muted" style="margin-top:8px">Hard limit: ${v.execution.eventCapPct}% ticket kapitálu · max ${v.execution.hardQtyCap} ks · nepřekročit ${v.execution.maxBuyPrice?money(v.execution.maxBuyPrice):'neznámý max BUY'}.</div></div>`).join('')||'<p class="muted">Aktuálně není žádný BUY TARGET vhodný k exekuci.</p>'}`;
