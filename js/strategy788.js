...
export async function buildStrategy788(s=store.get()){
 ...
 const months=new Set(A(s.personalSpending?.transactions).map(x=>String(x.date||'').slice(0,7)).filter(Boolean));
 const monthlySpend=Math.max(1,A(s.personalSpending?.transactions).filter(x=>N(x.amount)<0).reduce((a,x)=>a+Math.abs(N(x.amount)),0)/Math.max(1,months.size));
