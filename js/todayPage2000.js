...
function render(){
 ...
 host.innerHTML='<div class="pr1320-today" data-os2-today data-product-home1300>' +
  '<div class="pr1300-head"><div><div class="pr1300-kicker">'+esc(today)+'</div><h1>'+greeting()+', Kamile.</h1><p>Co máš řešit teď a stručný stav věcí, které OS hlídá.</p></div><span class="pr1300-status '+(items.some(x=>x.tone==='bad')?'bad':items.length?'warn':'good')+'">'+(items.length?items.length+' kroků':'hotovo')+'</span></div>' +
  ...
  '<section class="pr1300-panel pr1326-system-panel"><div class="pr1300-panel-head"><h2>Stav systému</h2><span>jedním klikem do detailu</span></div>'+systemRows(system)+'</section>' +
