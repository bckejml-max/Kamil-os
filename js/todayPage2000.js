...
function data(){
 ...
 const rawCash=s.financePlan?.cashNow;
 const cash=rawCash!==null&&rawCash!==undefined&&String(rawCash).trim()!==''&&Number.isFinite(Number(rawCash))?Number(rawCash):null;
...
function nextPriority(d){
 ...
 if(overdue)return{title:titleOf(overdue),why:'Má prošlý termín. Vyřešení sníží okamžité riziko.',action:'inbox',taskId:overdue.id,tone:'bad'};
 if(d.urgentTasks[0])return{title:titleOf(d.urgentTasks[0]),why:ts(d.urgentTasks[0])?`Nejbližší termín ${fmtDate(d.urgentTasks[0])}.`:'Nejvýše postavený otevřený úkol.',action:'inbox',taskId:d.urgentTasks[0].id,tone:'hot'};
...
function rows(items,{limit=5,side=fmtDate,empty='Nic otevřeného.',tasks=false}={}){
 if(!items.length)return`<div class="os2-empty">${esc(empty)}</div>`;
 return items.slice(0,limit).map(x=>`<div class="os2-row"><div class="os2-row-main"><b>${esc(titleOf(x))}</b><small>${esc(x?.category||x?.area||x?.project||x?.source||'')}</small></div><div class="os2-row-side">${esc(side(x))}${tasks?`<button class="os2-action-link" data-os2-task="${esc(x.id)}">Vyřešit</button>`:''}</div></div>`).join('');
}
...
function render(){
 ...
 host.innerHTML=`<div class="os2-today" data-os2-today>
  <div class="os2-welcome">
   <section class="os2-hero">
    <div><div class="os2-kicker">${esc(today)}</div><h1>${greeting()}, Kamile.</h1><p>Jedna obrazovka pro to důležité. Úkoly, termíny a nejbližší kroky na jednom místě.</p></div>
   ...
   <section class="os2-now">
    ...
    <div class="os2-now-actions">${priority.action?`<button class="os2-primary" ${priority.taskId?`data-os2-task="${esc(priority.taskId)}"`:`data-os2-nav="${esc(priority.action)}"`}>Otevřít</button>`:''}<button class="os2-icon-btn" data-os2-add>＋ Přidat</button></div>
   </section>
  </div>
  <div class="os2-kpis">
   ...
   <div class="os2-kpi"><span>Hotovost</span><b>${d.cash!==null?money(d.cash):'—'}</b><small>${d.cash!==null?'Zadaná volná hotovost':'Hotovost není zadaná'}</small></div>
  </div>
  <div class="os2-grid">
   <div class="os2-stack">
    <section class="os2-panel"><div class="os2-panel-head"><h3>Co řešit</h3><button class="os2-action-link" data-os2-nav="inbox">Otevřít Inbox</button></div><div class="os2-list">${rows(d.urgentTasks,{limit:6,tasks:true,empty:'Žádné otevřené úkoly.'})}</div></section>
 ...
 if(!host.dataset.os2Bound){host.dataset.os2Bound='1';ownEvent1100(OWNER,host,'click',async e=>{const taskButton=e.target?.closest?.('[data-os2-task]');if(taskButton){const task=(store.get().tasks||[]).find(x=>String(x.id)===taskButton.dataset.os2Task);if(task){const {openPersonalAction641}=await import('./personalActionExecution641.js');await openPersonalAction641({id:`task:${task.id}`,kind:'task',title:titleOf(task),why:`Termín: ${fmtDate(task)}`,next:task.notes||'Dokončit nebo posunout termín.',route:'today'})}return}const nav=e.target?.closest?.('[data-os2-nav]');if(nav){window.dispatchEvent(new CustomEvent('kamil:navigate',{detail:nav.dataset.os2Nav}));return}if(e.target?.closest?.('[data-os2-add]'))window.dispatchEvent(new CustomEvent('kamil:capture',{detail:'task'}))})}
