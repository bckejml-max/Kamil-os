...
function baseData(){
 ...
 const overdue=tasks.filter(x=>ts(x)&&ts(x)<Date.now()),transfer=tickets.filter(x=>['SOLD_UNDELIVERED','TRANSFER_REQUIRED','SOLD_WAITING_TRANSFER'].includes(upper(x.market_status||x.workflow))),activeTickets=tickets.filter(x=>!['PAID','PAYOUT_RECEIVED','SOLD'].includes(upper(x.market_status||x.workflow))),rawCash=s.financePlan?.cashNow,cash=rawCash!==null&&rawCash!==undefined&&String(rawCash).trim()!==''&&Number.isFinite(Number(rawCash))?Number(rawCash):null,work=workCommandCenter440(s),property=buildPropertyHub620(s),bet=betting();
...
function todayRows(items){if(!items.length)return '<div class="pr1300-empty">Žádné otevřené úkoly.</div>';return items.slice(0,6).map(x=>`<button type="button" class="pr1300-row pr1300-task-row" data-today1300-task="${esc(x.id)}"><div class="pr1300-row-main"><b>${esc(titleOf(x))}</b><small>${esc(x?.project||x?.area||x?.category||'')}</small></div><div class="pr1300-row-side ${ts(x)&&ts(x)<Date.now()?'bad':''}">${esc(fmtDate(x))}</div></button>`).join('')}
