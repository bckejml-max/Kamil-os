...
const delayOwned=(key,ms)=>new Promise(resolve=>schedule1100(OWNER,key,resolve,ms,{pauseWhenHidden:true}));
const yieldMain=()=>delayOwned(`yield-${Date.now()}`,LEGACY_YIELD_MS);
...
async function waitCanonicalAnalytics(){
  for(let i=0;i<12;i++){
    ...
    await delayOwned(`analytics-wait-${i}`,120);
  }
...
function scheduleEarlyBackground(state,delay=EARLY_DELAY_MS){
  ...
  earlyPromise=delayOwned('early-start',delay).then(()=>loadEarlyBackground(state)).catch(error=>{state.earlyError=String(error?.message||error);console.warn('[tickets466] early background failed',error);publishBoot(state);return false}).finally(()=>{earlyPromise=null});
  return earlyPromise;
}
...
async function loadLegacy(state){
  ...
  schedule1100(OWNER,'background-start',()=>{if(isTicketViewActive())loadBackground(state).catch(error=>{state.backgroundError=String(error?.message||error);console.warn('[tickets466] background analytics failed',error);publishBoot(state)});else{state.backgroundDeferred=true;publishBoot(state)}},1000,{pauseWhenHidden:true});return true
}
function scheduleLegacy(state,delay=LEGACY_DELAY_MS){
  ...
  legacyPromise=delayOwned('legacy-start',delay).then(()=>loadLegacy(state)).catch(error=>{console.error('[tickets466] deferred analytics failed',error);state.legacyError=String(error?.message||error);publishBoot(state);return false}).then(ok=>{legacyPromise=null;if(!ok&&!state.legacyDone&&isTicketViewActive()&&(state.legacyRetries||0)<MAX_LEGACY_RETRIES){state.legacyRetries=(state.legacyRetries||0)+1;schedule1100(OWNER,'legacy-retry',()=>scheduleLegacy(state,LEGACY_RETRY_MS),LEGACY_RETRY_MS,{pauseWhenHidden:true})}return ok});
  return legacyPromise
}
function bindWake(){
  if(wakeBound)return;wakeBound=true;
  ownEvent1100(OWNER,window,'kamil:view-change',()=>{const state=window.__KAMIL_TICKET_BOOT466__;if(!state||!isTicketViewActive())return;if(!state.earlyDone)scheduleEarlyBackground(state,40);if(!state.legacyDone)scheduleLegacy(state,400);else if(!state.backgroundDone)loadBackground(state).catch(error=>{state.backgroundError=String(error?.message||error);publishBoot(state)})});
}
async function desk(){
  const state={version:BOOT_VERSION,startedAt:Date.now(),finishedAt:null,status:'STARTING',modules:[],failed:[],ok:0,criticalDone:false,earlyDone:false,legacyStarted:false,legacyDone:false,backgroundDone:false,legacyRetries:0,prunedLegacy:true,runtimeOwner:OWNER};
...
export async function renderTicketPage100(){
 ...
 if(!sess){
  ...
  if(host){
   host.innerHTML='<section class="os2-connection-state"><div class="eyebrow">VSTUPENKY</div><h1>Připoj své portfolio vstupenek</h1><p>Vstupenky se načítají z tvého cloudového účtu. V tomto prohlížeči zatím nejsi přihlášený — prázdná obrazovka neznamená, že se tvoje vstupenky smazaly.</p><button class="btn primary" data-ticket-connect>Připojit cloudový účet</button><p class="muted">Po přihlášení můžeš zobrazit portfolio, přidávat vstupenky a importovat evidenci.</p></section>';
   host.querySelector('[data-ticket-connect]').onclick=()=>window.dispatchEvent(new CustomEvent('kamil:cloud-login'));
  }
  return {healthy:true,requiresSession:true};
 }
 if(!bootPromise)bootPromise=desk().catch(error=>{bootPromise=null;legacyPromise=null;earlyPromise=null;const state=window.__KAMIL_TICKET_BOOT466__||{version:BOOT_VERSION,modules:[]};state.status='FATAL';state.fatal=String(error?.message||error);state.finishedAt=Date.now();window.__KAMIL_TICKET_BOOT466__=state;document.documentElement.dataset.ticketBoot466='fatal';console.error('[tickets466] base desk boot failed',error);throw error});
 await bootPromise;
 if(!document.querySelector('#ticketIntelView .td331'))window.dispatchEvent(new CustomEvent('kamil:view-change',{detail:'tickets'}));
 return window.__KAMIL_TICKET_DESK331__;
}
