...
export function renderPersonalFamily640(){
 ...
 <section class="card family-section" data-family-section="events"><div class="eyebrow">NEJBLIŽŠÍ TERMÍNY</div>${events.length?events.map((x,i)=>row(x.title||x.summary||'Událost',when(x),`<button class="btn ${x.d<=1?'primary':''}" data-family-event="${i}">Připravit</button>`)).join(''):'<div class="empty success-empty">V příštích 30 dnech nemám rodinný termín.</div>'}</section>
 <section class="card family-section" data-family-section="tasks"><div class="eyebrow">RODINNÉ ÚKOLY</div>${tasks.length?tasks.map((x,i)=>row(x.title||'Úkol',x.due?`termín ${new Date(x.due).toLocaleDateString('cs-CZ')}`:'bez termínu',`<button class="btn" data-family-task="${i}">Řešit</button>`)).join(''):'<div class="empty success-empty">Žádný otevřený rodinný úkol.</div>'}</section>
