...
export function ticketRiskBudgetCandidate202(candidate={},exposure={},capitalCzk=0,opts={}){
 ...
 const maxBuy=n(candidate.maxBuyPrice);
 const buyPrice=price>0&&maxBuy>0?Math.min(price,maxBuy):price||maxBuy||0;
 const hardQtyCap=Math.max(1,Math.floor(n(opts.hardQtyCap)||8));
 const guard=ticketExposureGuard201(candidate,exposure,opts);
 const baseBudget=capital>0?Math.floor(capital*.20/10)*10:0;
 ...
 return {...candidate,riskBudget:{version:TICKET_RISK_BUDGET_VERSION_202,verdict,buyPrice:buyPrice||null,maxQty,baseBudget:baseBudget||null,allowedBudget:allowedBudget||null,deployCapital:maxQty&&buyPrice?Math.round(maxQty*buyPrice):null,binding:guard.binding||null,guard}};
}
...
export function buildTicketRiskBudget202({inventory=[],latest=new Map(),watchlist=[],ticketBook={}}={},now=Date.now(),opts={}){
 ...
 return {version:TICKET_RISK_BUDGET_VERSION_202,capital,exposure,rows,buy:rows.filter(x=>x.riskBudget.verdict==='BUY'),blocked:rows.filter(x=>['CONCENTRATED','TOO LARGE'].includes(x.riskBudget.verdict)),summary:{candidates:rows.length,buy:rows.filter(x=>x.riskBudget.verdict==='BUY').length,blocked:rows.filter(x=>['CONCENTRATED','TOO LARGE'].includes(x.riskBudget.verdict)).length,setCapital:rows.filter(x=>x.riskBudget.verdict==='SET CAPITAL').length}};
}
