...
export function personalActions640(s=store.get()){
 ...
 const push=x=>rows.push({...x,level:level(x.score)});
 ...
 for(const e of (s.calendar?.events||[]).filter(personal)){
  ...
  push({id:`calendar:${e.id||e.title}`,score:d===0?112:d===1?100:d<=3?88:70,title:e.title||e.summary||'Událost',why:`Kalendář · ${whenLabel(d)}`,next:'Připravit se na událost.',minutes:5,kind:'calendar',route:'tickets'});
 }
