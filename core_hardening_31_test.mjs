...
const version=release.match(/APP_VERSION='([^']+)'/)?.[1],releaseLine=release.match(/APP_RELEASE='([^']+)'/)?.[1];
assert(/^31\.\d+\.\d+$/.test(version||'')&&releaseLine===version.split('.').slice(0,2).join('.'),'releaseMeta must provide one internally consistent 31.x version');
...
assert(index.includes('<title>Kamil OS 31.')&&index.includes('systemHealthUi31.js'),'31.x shell/system health missing');
...
assert(sw.includes("const CACHE='kamil-os-31.")&&sw.includes('./js/systemHealth31.js'),'31.x service-worker shell missing');
assert(vercel.includes('Content-Security-Policy')&&vercel.includes('X-Content-Type-Options'),'security headers missing');
console.log('CORE HARDENING 31 REGRESSION TEST PASS');
