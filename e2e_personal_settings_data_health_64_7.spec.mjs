...
test('64.7 exposes personal data health and safe priority bias',async({page})=>{
 ...
 const out=await page.evaluate(async()=>{
  ...
  return{area:health.priorityArea,coverage:health.coverage,records:health.records,top:queue.top3.map(x=>({title:x.title,area:x.area,score:x.score})),hasWork:String(settings.exportPersonalData647).includes('xtbReport')};
 });
