...
test('Assistant 53.0 is private, click-only and action-first',async({page})=>{
 ...
 expect(r.cat).toBe('Auto');
 expect(r.search.some(x=>x.title.includes('pojistku'))).toBe(true);
 expect(r.search.some(x=>x.title.includes('PKS'))).toBe(false);
 expect(r.safe.limit).toBeGreaterThanOrEqual(0);
 expect(r.ticket[0].name).toBe('Koncert A');
