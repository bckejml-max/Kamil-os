...
test('Final Verdict 55.8 blocks actionable decisions with stale/conflicting data',async({page})=>{
 ...
 const result=await page.evaluate(async()=>{const m=await import('./js/finalMarketVerdict558.js'),s=JSON.parse(localStorage.getItem('kamil-os-state'));return m.finalMarketVerdict558(s)});
 expect(result.total).toBeGreaterThan(0);
 expect(result.blocked.length).toBeGreaterThan(0);
 expect(result.blocked.every(x=>x.verdict==='DO NOT ACT YET')).toBe(true);
 expect(result.rows.every(x=>['BUY','HOLD','SELL','REPRICE','WAIT','DO NOT ACT YET'].includes(x.verdict))).toBe(true);
 const diag=await page.evaluate(()=>window.__KAMIL_FINAL_VERDICT_558_LAST__);expect(diag.ms).toBeLessThan(500);
});

test('Final Verdict opens only after explicit click through market decision chain',async({page})=>{
 await page.goto(BASE,{waitUntil:'domcontentloaded'});
 expect(await page.evaluate(()=>window.__KAMIL_FINAL_VERDICT_558_LAST__||null)).toBeNull();
