...
test('capture canonical visual audit',async({page})=>{
 fs.mkdirSync('visual-audit1325',{recursive:true});

 await page.setViewportSize({width:1440,height:1000});
 await boot(page);
 for(const view of VIEWS){
  if(view!=='today')await openView(page,view,'#mainNav');
  await page.screenshot({path:`visual-audit1325/desktop-${view}.png`,fullPage:true});
 }
 ...
 for(const view of VIEWS){
  ...
  await page.screenshot({path:`visual-audit1325/mobile-${view}.png`,fullPage:true});
 }
});