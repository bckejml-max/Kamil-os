...
for(const f of ['js/legacyCleanup987.js','js/legacyCleanup987Boot.js','js/oneOS977.js','js/consolidationRuntime.js'])need(fs.existsSync(f),`missing ${f}`);
...
const c=read('js/legacyCleanup987.js'),runtime=read('js/consolidationRuntime.js'),betting=read('js/bettingBootstrap543.js');
...
need(runtime.includes("install('./legacyCleanup987.js','installLegacyCleanup987')"),'OS987 central bootstrap not wired');
need(!betting.includes('legacyCleanup987'),'betting bootstrap must not own legacy cleanup');
