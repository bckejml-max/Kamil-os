...
const bootstrap=read('js/bettingBootstrap543.js');assert.ok(bootstrap.includes("const REV='os695'"),'OS695 bootstrap cache-bust revision missing');assert.ok(bootstrap.indexOf('await ensureBrowserFeed();')<bootstrap.indexOf('await ensureFeed();'),'browser feed must install before external providers');assert.ok(bootstrap.includes('boot().then(ok=>{if(!ok)installBootObserver()})'),'async bootstrap fallback is not awaited');
const browser=read('js/bettingBrowserFeed694.js');assert.ok(browser.includes('KAMIL_CHANCE_FEED_694'),'browser relay event missing');assert.ok(browser.includes("window.fetch=fetch695"),'browser feed fetch switch missing');assert.ok(browser.includes('/chance-feeder695.user.js'),'userscript install link missing');
const user=read('chance-feeder695.user.js');assert.ok(user.includes('@match        https://www.chance.cz/*'),'Chance userscript match missing');assert.ok(user.includes('scrapeVisible()'),'Chance visible-odds capture missing');assert.ok(user.includes('chance_browser_model694'),'browser model endpoint missing');assert.ok(user.includes('GM_setValue'),'cross-origin userscript relay storage missing');
