...
const LEAGUES=[
 ...
 {code:'D2',name:'Germany 2 Bundesliga',rules:[['nemeck','2 bundesliga'],['nemeck','2. bundesliga'],['germany','2 bundesliga'],['german','2 bundesliga']]},
...
export function parseFootballDataCsv(text){
 ...
 for(const line of lines.slice(1)){
  ...
  const homeGoals=Number(row[index.get('FTHG')]);
  const awayGoals=Number(row[index.get('FTAG')]);
  if(!home||!away||!Number.isFinite(homeGoals)||!Number.isFinite(awayGoals))continue;
  matches.push({
   home,away,homeGoals,awayGoals,
   homeCorners:index.has('HC')?Number(row[index.get('HC')]):null,
   awayCorners:index.has('AC')?Number(row[index.get('AC')]):null,
   homeYellow:index.has('HY')?Number(row[index.get('HY')]):null,
   awayYellow:index.has('AY')?Number(row[index.get('AY')]):null,
   homeRed:index.has('HR')?Number(row[index.get('HR')]):null,
   awayRed:index.has('AR')?Number(row[index.get('AR')]):null
  });
 }
 return matches;
}
...
const TEAM_ALIASES=new Map([
 ...
 ['ac milan','milan'],['bayern munchen','bayern munich'],['bayern munich','bayern munich'],
 ['borussia monchengladbach','m gladbach'],['monchengladbach','m gladbach'],['atletico madrid','ath madrid'],
 ['athletic bilbao','ath bilbao'],['athletic club','ath bilbao'],['real betis balompie','betis'],['real sociedad de futbol','sociedad']
]);
