...
export async function resolveFootballDataModels(events,options={}){
 ...
 const meta={requested:true,provider:'football-data-poisson',configured:true,limit,candidateEvents:candidates.length,matchedEvents:0,modeledEvents:0,modeledSelections:0,dataRequests:0,cacheHits:0,handicapRule:'FULL_TIME half-goal only; probability=min(team-specific Poisson, league-neutral Poisson)',leagues:[],errors:[],matches:[]};
 ...
 for(const item of candidates){
  ...
  const neutralModel=poissonMarkets(lambdas.leagueHome,lambdas.leagueAway);
  let modeled=0;
  for(const market of Array.isArray(event?.markets)?event.markets:[]){
   ...
   for(const selection of Array.isArray(market?.selections)?market.selections:[]){
    ...
    let p=selectionProbability(market,selection,model);
    if(!Number.isFinite(p)||p<=0||p>=1)continue;
    let source='football-data-poisson';
    if(type==='ASIAN_HANDICAP'){
     const neutral=selectionProbability(market,selection,neutralModel);
     if(!Number.isFinite(neutral)||neutral<=0||neutral>=1)continue;
     p=Math.min(p,neutral);
     source='football-data-poisson-handicap-conservative';
    }
    probabilities.set(String(selection.id),p);
    sources.set(String(selection.id),source);
    modeled+=1;
   }
  }
  if(modeled){
