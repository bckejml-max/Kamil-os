...
function marketMetric(market){
 ...
 const explicitYellow=type.includes('YELLOW')||(text.includes('yellow')&&text.includes('card'))||(text.includes('zlut')&&text.includes('kart'));
...
export async function resolveFootballDataCountModels(events,options={}){
 ...
 for(const {event,league} of candidates){
  ...
  for(const market of event.markets){
   ...
   if(!diagnostics.has(metric))diagnostics.set(metric,{metric,markets:0,selections:0,modeledSelections:0,...metricHistoryStats(matches,metric,home.name,away.name),lambdaAvailable:false,lambdaHome:null,lambdaAway:null});
