...
function hasCountSelection(market){
...
function marketMetric(market){
 const rawPeriod=String(market?.period||'').trim();
 if(rawPeriod&&rawPeriod.toUpperCase()!=='FULL_TIME')return null;
 const type=String(market?.type||'').toUpperCase();
 if(!type.includes('OVER_UNDER')||!hasCountSelection(market))return null;
...
export async function resolveFootballDataCountModels(events,options={}){
 ...
 const meta={requested:true,provider:'football-data-count-calibrated',configured:true,limit,candidateEvents:candidates.length,matchedEvents:0,modeledEvents:0,modeledSelections:0,dataRequests:0,cacheHits:0,calibrationWeights:COUNT_WEIGHTS,calibrationStrategy:'downward-only-prior-shrinkage',neverIncreasesProbability:true,blockedIncreases:0,scopeRule:'FULL_TIME (or legacy unspecified period) over-under corners + explicit yellow-card over-under markets only',leagues:[],errors:[],matches:[],metricDiagnostics:[]};
