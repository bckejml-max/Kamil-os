...
function marketMetric(market){
 ...
 if((text.includes('yellow')&&text.includes('card'))||text.includes('zluta karta')||text.includes('zlute karty'))return 'yellow_cards';
 return null;
}
...
export async function resolveFootballDataCountModels(events,options={}){
 ...
 const meta={requested:true,provider:'football-data-count',configured:true,limit,candidateEvents:candidates.length,matchedEvents:0,modeledEvents:0,modeledSelections:0,dataRequests:0,cacheHits:0,leagues:[],errors:[],matches:[]};
 ...
 for(const {event,league} of candidates){
  ...
  for(const market of event.markets){
   ...
   for(const selection of Array.isArray(market?.selections)?market.selections:[]){
    ...
    const p=probabilityFor(selection,market,lambdas);
    if(!Number.isFinite(p)||p<=0||p>=1)continue;
    probabilities.set(String(selection.id),p);
    sources.set(String(selection.id),`football-data-count-${metric}`);
    modeled+=1;
   }
  }
