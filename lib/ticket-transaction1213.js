...
export const TICKET_TRANSACTION_VERSION='1215.0.0';
...
function revisionOf(ticket){const value=Number(ticket?.revision??0);if(!Number.isInteger(value)||value<0)fail('TICKET_REVISION_INVALID',{revision:ticket?.revision});return value}
export function prepareTicketTransaction1213(current,event,{now=Date.now()}={}){
 ...
 const expectedRevision=revisionOf(base),appended=appendTicketEvent1195(stream,event);
 if(appended.replay)return{version:TICKET_TRANSACTION_VERSION,replay:true,committed:false,original,next:original,eventId:event?.eventId||event?.messageId||event?.id||null,expectedRevision,nextRevision:expectedRevision};
 ...
 const counts={purchased:event?.purchased??base.purchased??base.qty??0,listed:event?.listed??base.listed??0,sold:event?.sold??(type==='buyer'?baseSold+qty:baseSold),transferred:event?.transferred??(type==='transfer_confirmed'?baseTransferred+qty:baseTransferred)};
 const quantity=reconcileQuantity1190(counts);if(!quantity.ok)fail('TICKET_QUANTITY_INVARIANT',{quantity});
 let target=nominalTarget;if(type==='transfer_confirmed'&&quantity.transferred<quantity.sold)target='transfer_required';
 ...
 const reduced=reduceTicketEvents1195(appended.stream,{...base,state:from}),viagogo=viagogoState1194(base.viagogoState,type),nextRevision=expectedRevision+1;
 const next={...base,...reduced,state:target,events:appended.stream,purchased:quantity.purchased,listed:quantity.listed,sold:quantity.sold,transferred:quantity.transferred,available:quantity.available,viagogoState:viagogo.state,revision:nextRevision,updatedAt:new Date(now).toISOString()};
 ...
 return{version:TICKET_TRANSACTION_VERSION,replay:false,committed:false,original,next,eventId:appended.stream.at(-1)?.eventId||null,quantity,payout,viagogo,expectedRevision,nextRevision};
}
export async function commitTicketTransaction1213({current,event,persist,checkpoint=null,checkpointKey=null,now=Date.now()}={}){
 ...
 let persisted=false,persistResult=null;const meta={expectedRevision:tx.expectedRevision,nextRevision:tx.nextRevision,eventId:tx.eventId};
 const checkpointCommit=await commitCheckpoint1199({key:checkpointKey||`ticket:${tx.eventId||'event'}`,checkpoint,persist:async()=>{persistResult=await persist(clone(tx.next),meta);if(persistResult?.conflict===true||persistResult?.stale===true)fail('TICKET_WRITE_CONFLICT',{...meta,persistResult});persisted=true}});
 if(!persisted)fail('TICKET_PERSIST_NOT_CONFIRMED');return{...tx,committed:true,checkpointCommit,persistResult};
}
