...
export async function resolveAutoBettingModels(events,options={}){
 ...
 for(const [id,p] of count.probabilities){
  ...
  sources.set(String(id),count.sources.get(String(id))||'football-data-count-calibrated');
 }
 for(const [id,p] of api.probabilities){
  probabilities.set(String(id),p);
  sources.set(String(id),'api-football');
 }
 const layers=['football-data-poisson-calibrated'];
 if(count.probabilities.size)layers.push('football-data-count-calibrated');
 if(apiKey)layers.push('api-football');
 const provider=layers.join('+');
 const meta={
  requested:true,
  provider,
  configured:true,
