...
export const TICKET_GMAIL_BATCH_VERSION='1217.0.0';
...
export function planTicketGmailBatch1215({messages=[],inventory=[],states=[],checkpoint=null,now=Date.now()}={}){
 ...
 const stateByTicket=new Map((states||[]).map(x=>[clean(x?.ticket_id||x?.ticketId),{payload:clone(x?.payload||x?.state||{}),revision:Math.max(0,Number(x?.revision||0)||0)}]));
 ...
 for(const [order,rows] of grouped){
  ...
  const durable=stateByTicket.get(ticketId),expectedRevision=durable?.revision||0;let current=durable?.payload;
  if(!current||!clean(current.state))current=bootstrap(ticket,clean(rows[0]?.type).toLowerCase());
  ...
  items.push({ticketId,orderId:order,expectedRevision,payload:current,patch:patchFor(ticket,current)});
 }
