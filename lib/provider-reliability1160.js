...
export async function parseProviderJson1160(response,{provider='UPSTREAM',allowArray=false,allowNull=false,requireJsonContentType=true}={}){const type=String(response?.headers?.get?.('content-type')||'').toLowerCase();if(requireJsonContentType&&!type.includes('application/json')&&!type.includes('+json'))throw providerError1160('NON_JSON',provider,{contentType:type||null,status:Number(response?.status)||0});const text=await response.text();return parseJsonText1160(text,{provider,allowArray,allowNull})}
