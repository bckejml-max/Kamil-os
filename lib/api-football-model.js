...
export async function resolveApiFootballModels(events,options={}){
 ...
 const allCandidates=(Array.isArray(events)?events:[]).filter(event=>matchResultMarket(event)).sort((a,b)=>Date.parse(a?.startTime||'')-Date.parse(b?.startTime||''));
 const fixtureDateLimit=Math.max(1,Math.min(3,Number.parseInt(String(options.fixtureDateLimit??2),10)||2));
 const allowedDates=[];
 for(const event of allCandidates){
  const date=utcDate(event.startTime);
  if(date&&!allowedDates.includes(date)&&allowedDates.length<fixtureDateLimit)allowedDates.push(date);
 }
 const candidates=allCandidates.filter(event=>allowedDates.includes(utcDate(event.startTime)));
 ...
 const meta={
  ...
  ignoredCandidateEvents:Math.max(0,allCandidates.length-candidates.length),
  fixtureDateLimit,
  fixtureDates:allowedDates,
  matchedEvents:0,
  modeledEvents:0,
  modeledSelections:0,
  apiRequests:0,
 ...
 for(const event of candidates){
  ...
  if(predictionAttempts>=limit){
   meta.matches.push({eventId:event?.id??null,fixtureId,status:'MATCHED_LIMIT_REACHED'});
   continue;
  }
 ...
 meta.predictionAttempts=predictionAttempts;
