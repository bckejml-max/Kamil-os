...
async function boot(page){
  ...
  await expect.poll(()=>page.evaluate(()=>window.__KAMIL_COMMAND_CENTER467__?.healthy),{timeout:5000}).toBe(true);
...
test('OS349 promotes urgent monthly manager duties into the global focus queue',async({page})=>{
  ...
  await page.evaluate(()=>{
    ...
    window.__KAMIL_COMMAND_CENTER467__?.refresh?.();
  });
  ...
  await expect.poll(()=>page.evaluate(()=>window.__KAMIL_COMMAND_CENTER467__?.model?.best?.key)).toBe('manager');
  const button=page.locator('[data-command-center467-root] [data-os467-open="manager"]');
