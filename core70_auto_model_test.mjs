...
assert.equal(data.value.autoModel.provider,'football-data-poisson-calibrated');
...
fetchCalls=[];
res=responseCapture();
await handler({method:'GET',url:'/api/core70-health?source=chance&sport=soccer&days=1&value=1&autoModel=1&poissonLimit=10&minOdds=1.45&maxOdds=3.20'},res);
data=res.json();
assert.equal(data.ok,true);
assert.equal(data.value.modelProviderConfigured,true);
assert.equal(data.value.modelProvider,'football-data-poisson-calibrated');
...
assert.ok(data.value.autoModel.calibration.calibratedSelections>=5);
assert.ok(fetchCalls.filter(url=>url.includes('football-data.co.uk')).length===2);
const allSelections=data.events.flatMap(event=>event.markets.flatMap(market=>market.selections));
assert.ok(allSelections.some(pick=>pick.modelSource==='football-data-poisson-calibrated'&&pick.modelProbability!==null));
const integerOver=allSelections.find(pick=>pick.id==='prem-o2');
const integerUnder=allSelections.find(pick=>pick.id==='prem-u2');
assert.equal(integerOver?.modelProbability,null);
assert.equal(integerUnder?.modelProbability,null);
assert.equal(integerOver?.decision,'WAITING_FOR_MODEL');
assert.equal(integerUnder?.decision,'WAITING_FOR_MODEL');
