...
const req={method:'POST',url:'https://kamil-os.test/api/ticket-market-watch',json:async()=>({items:[{id:'x',label:'Davis Cup - 405',section:'405',qty:3,buyEachCzk:1800,status:'NOT_LISTED',viagogoUrl:'https://www.viagogo.com/ww/E-1'}]})};
...
assert.equal(status,200);assert.equal(payload.ok,true);assert.equal(payload.results.length,1);assert.equal(payload.results[0].market.confidence,'section');assert.equal(payload.results[0].market.price,180);assert.equal(payload.results[0].market.priceCzk,3870);assert.equal(payload.results[0].market.listingCount,25);assert.equal(payload.results[0].market.supplyPct,2);assert.equal(payload.results[0].recommendation.label,'VYSTAVIT');assert.match(headers['Cache-Control'],/s-maxage/);
