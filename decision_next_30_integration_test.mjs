...
const s={
 ...
 // Deliberately incomplete market snapshot: no verified live quote / basis contract.
...
// OS148+ safety: incomplete XTB data must never be asserted as an actionable TRIM/BUY/SELL decision by this integration test.
if(xtb){
 assert(!['BUY','TRIM','SELL'].includes(String(xtb.action||'').toUpperCase()),'incomplete XTB data must be blocked from actionable decision');
 const xn=decisionNext30(xtb);
 if(['REVIEW','VERIFY_DATA','PROVĚŘIT DATA'].includes(String(xtb.action||'').toUpperCase()))assert(!xn.rows.some(x=>['BUY','SELL'].includes(x.kind)),'review decision must not invent executable buy/sell trigger');
}
...
// Standalone Next Trigger remains read-only: only echoes rules that are explicitly supplied by the source decision.
