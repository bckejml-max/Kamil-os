...
test('Kamil OS 32.2 local-first auth and critical flow',async({page})=>{
  const requests=[];page.on('request',r=>requests.push(r.url()));
  await page.goto(BASE,{waitUntil:'networkidle'});
  await expect(page).toHaveTitle(/Kamil OS 32\.2/);
...
test('Kamil OS 32.2 mirrors selected history into IndexedDB',async({page})=>{
...
test('Kamil OS 32.2 cloud history mapper is allowlisted and non-destructive',async({page})=>{
  await page.goto(BASE,{waitUntil:'networkidle'});
  const result=await page.evaluate(async()=>{const {cloudHistoryRows32,cloudHistory32Info}=await import('./js/cloudHistory32.js');return {rows:cloudHistoryRows32([{key:'ticket|1',bucket:'ticket',payload:{profit:1}},{key:'vault|1',bucket:'vault',payload:{secret:'x'}}],'user','32.2.0','2026-08-21T00:00:00Z'),info:cloudHistory32Info}});
...
test('Kamil OS 32.2 creates a safe local item-level outbox operation',async({page})=>{
  const requests=[];page.on('request',r=>requests.push(r.url()));
  await page.addInitScript(()=>{
...
test('Kamil OS 32.2 confirmed merge engine never auto-applies conflicts or deletes',async({page})=>{
  await page.goto(BASE,{waitUntil:'networkidle'});
...
test('Kamil OS 32.2 Copilot write waits for explicit confirmation',async({page})=>{
  await page.addInitScript(()=>{
