...
const budgets={timers:394,listeners:719,intervals:31};
const targets={timers:350,listeners:650,intervals:0};
assert.ok(totals.timers<=budgets.timers,`OS1290 timer budget regression: ${totals.timers} > ${budgets.timers}`);
assert.ok(totals.listeners<=budgets.listeners,`OS1290 listener budget regression: ${totals.listeners} > ${budgets.listeners}`);
assert.ok(totals.intervals<=budgets.intervals,`OS1290 interval budget regression: ${totals.intervals} > ${budgets.intervals}`);
...
for(const name of ['bettingRequestBudget561.js','bettingTiming564.js','bettingPerformance565.js','bettingIntelligence560.js','bettingMissed566.js']){
...
for(const name of ['ticketPredictive433.js','ticketBacktest434.js','ticketCommander435.js','ticketComparable437.js','ticketRisk438.js']){
 const src=await readFile(new URL(`./js/${name}`,root),'utf8');
 assert.equal((src.match(/setInterval\s*\(/g)||[]).length,0,`OS1290 ${name} must not use raw polling intervals`);
 assert.match(src,/schedule1100/,`OS1290 ${name} must use OS1100 recurring scheduling`);
 assert.match(src,/pauseWhenHidden/,`OS1290 ${name} recurring work must pause while hidden`);
}
...
console.log(JSON.stringify({name:'OS1290 Ticket analytics interval ownership contract',totals,budgets,targets,topHotspots:hotspots.slice(0,20),intervalHotspots},null,2));