...
assert.match(api,/uniqueIds\(list\?\.messages\)|uniqueIds\(list\.messages\)/,'OS1152 full Gmail source ids must be deduplicated before message detail fetches');
assert.match(api,/uniqueIds\(plan\.ids\)/,'OS1200 incremental Gmail source ids must be deduplicated before message detail fetches');
assert.match(api,/messages=uniqueRows\(rows\)|items=uniqueRows\(rows\)|tickets=uniqueRows\(rows\)/,'OS1152 parsed Gmail rows must be deduplicated before response');
...
console.log('OS1152/1153/1200 Gmail replay, incremental dedupe and checkpoint guard PASS');
