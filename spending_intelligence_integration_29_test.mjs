...
assert(s.meta.schemaVersion===SCHEMA_VERSION,`Spending Intelligence state migrates to canonical schema ${SCHEMA_VERSION}`);assert(Array.isArray(s.netWorthBook.items)&&Array.isArray(s.netWorthBook.history),'net worth collections coexist with spending');assert(s.projects[0].name==='Legacy work','legacy state preserved');
...
console.log('SPENDING INTELLIGENCE INTEGRATION PASS');
