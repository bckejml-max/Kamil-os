...
a=personalCopilot30('Čisté jmění',base,{},now);assert(a?.intent==='NET_WORTH'&&a.lines.some(x=>x.includes('aktiva'))&&a.lines.some(x=>x.includes('závazky')),'net worth answer');
...
a=personalCopilot30('Tento měsíc',base,{},now);assert(a?.intent==='MONTH_REVIEW'&&a.lines.length>0,'monthly review composed');
a=personalCopilot30('Co příští měsíc?',base,{},now);assert(a?.intent==='NEXT_MONTH'&&a.lines.length>0,'next month composed');
a=personalCopilot30('12 měsíců dopředu',base,{},now);assert(a?.intent==='YEAR_AHEAD'&&a.lines.some(x=>x.includes('Otevřeno')),'year ahead composed from real milestone');
