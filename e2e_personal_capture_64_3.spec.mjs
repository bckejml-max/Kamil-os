...
const openAdd=async page=>{await page.waitForFunction(()=>window.__KAMIL_PERSONAL_SHELL_BOUND__===true);await page.locator('#quickAddBtn').click();const dialog=page.getByRole('dialog').filter({hasText:'Přidat osobní věc'});await expect(dialog).toBeVisible({timeout:10000});await expect(dialog.locator('select[name="type"]')).toBeVisible();return dialog;};

test('Quick Add creates a personal waiting item',async({page})=>{
 await page.goto(BASE,{waitUntil:'domcontentloaded'});
 const dialog=await openAdd(page);await expect(dialog).toContainText('Přidat osobní věc');
