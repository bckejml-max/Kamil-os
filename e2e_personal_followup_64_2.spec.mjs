...
test('64.2 moves an active task into Waiting without duplicate priority',async({page})=>{
 ...
 await page.goto(BASE,{waitUntil:'domcontentloaded'});
 const out=await page.evaluate(async()=>{const f=await import('./js/personalFollowup642.js');const a=await import('./js/personalActions640.js');f.markPersonalWaiting642({id:'task:t1',title:'Čekám na potvrzení pojišťovny',kind:'task'},3);const s=JSON.parse(localStorage.getItem('kamil-os-state')||'{}');const actions=a.personalActions640();return{task:s.tasks[0],delegations:s.delegations||[],matching:actions.all.filter(x=>x.title==='Čekám na potvrzení pojišťovny')}});
 expect(out.task.waitingFor).toBe(true);expect(out.delegations).toHaveLength(1);expect(out.matching).toHaveLength(1);expect(out.matching[0].kind).toBe('waiting');
});
