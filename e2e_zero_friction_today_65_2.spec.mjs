...
test('65.2 one-tap tomorrow snoozes personal work to 09:00 and removes it from Today',async({page})=>{
 await page.goto(BASE,{waitUntil:'domcontentloaded'});
 const out=await page.evaluate(async()=>{
  ...
  const before=personalDailyAssistant650(store.get());
  const ok=postponePersonalActionToTomorrow642({id:'task:snooze-652',kind:'task',title:'Test zítra'});
  const row=store.get().tasks.find(x=>x.id==='snooze-652'),after=personalDailyAssistant650(store.get());
  return{ok,due:row?.due,status:row?.status,beforeTop:before.top.some(x=>x.id==='task:snooze-652'),afterTop:after.top.some(x=>x.id==='task:snooze-652'),tomorrow:after.tomorrow.some(x=>x.id==='snooze-652')};
 });
 expect(out.ok).toBe(true);expect(out.status).toBe('OPEN');expect(out.beforeTop).toBe(true);expect(out.afterTop).toBe(false);expect(out.tomorrow).toBe(true);const due=new Date(out.due);expect(due.getHours()).toBe(9);expect(due.getTime()).toBeGreaterThan(Date.now());
});
