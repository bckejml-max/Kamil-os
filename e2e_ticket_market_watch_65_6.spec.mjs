...
test('65.6 local Market Watch remains a safe fallback when 66.0 cloud is unavailable',async({page})=>{
 ...
 await expect.poll(()=>page.evaluate(()=>JSON.parse(localStorage.getItem('kamil-os-state')||'{}').ticketBook?.items?.find(x=>x.id==='listed-a')?.listPrice)).toBe(1700);
});
